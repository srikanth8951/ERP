...
class Profile extends ResourceController
{
	...
	{
		$response = array();
		$this->validatePermission('view_rsd_Head');	// Check permission
		...
		if ($userDetails) {
			...
			$response['message'] = lang('Employee.rsd.success_detail');
			$response['employee'] = [
				'type' => $this->empType,
				'data' => $userDetails
			];
			...
			$response['message'] = lang('Employee.rsd.error_detail');
			return $this->setResponseFormat("json")->respond($response, 201);
		}
	}
	...
	public function editEmployee()
	{
		...
		$this->validatePermission('edit_rsd');	// Check permission
		...
		if ($this->validate($rules, $messages)) {
			...
			if ($employee) {
				...
				$rsd = $modelEmployee->getEmployeeByValidation($filter_data);
				if ($rsd) {
					$response['status'] = 'error';

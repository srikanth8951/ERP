...
class Profile extends ResourceController
{
	...
	{
		$response = array();
		$this->validatePermission('view_store_Head');	// Check permission
		...
		if ($userDetails) {
			...
			$response['message'] = lang('Employee.store.success_detail');
			$response['employee'] = [
				'type' => $this->empType,
				'data' => $userDetails
			];
			...
			$response['message'] = lang('Employee.store.error_detail');
			return $this->setResponseFormat("json")->respond($response, 201);
		}
	}
	...
	public function editEmployee()
	{
		...
		$this->validatePermission('edit_store');	// Check permission
		...
		if ($this->validate($rules, $messages)) {
			...
			if ($employee) {
				...
				$store = $modelEmployee->getEmployeeByValidation($filter_data);
				if ($store) {
					$response['status'] = 'error';

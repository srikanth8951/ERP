...
namespace App\Controllers\Employee\regionalHead;
...
class Profile extends ResourceController
{
	...
	{
		$response = array();
		$this->validatePermission('view_regionalHead_Head');	// Check permission
		...
		if ($userDetails) {
			...
			$response['message'] = lang('Employee.regionalHead.success_detail');
			$response['employee'] = [
				'type' => $this->empType,
				'data' => $userDetails
			];
			...
			$response['message'] = lang('Employee.regionalHead.error_detail');
			return $this->setResponseFormat("json")->respond($response, 201);
		}
	}
	...
	public function editEmployee()
	{
		...
		$this->validatePermission('edit_regionalHead');	// Check permission
		...
		if ($this->validate($rules, $messages)) {
			...
			if ($employee) {
				...
				$regionalHead = $modelEmployee->getEmployeeByValidation($filter_data);
				if ($regionalHead) {
					$response['status'] = 'error';

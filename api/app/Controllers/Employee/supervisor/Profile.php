...
namespace App\Controllers\Employee\supervisor;
...
class Profile extends ResourceController
{
	...
	{
		$response = array();
		$this->validatePermission('view_cam_Head');	// Check permission
		...
		if ($userDetails) {
			...
			$response['message'] = lang('Employee.cam.success_detail');
			$response['employee'] = [
				'type' => $this->empType,
				'data' => $userDetails
			];
			...
			$response['message'] = lang('Employee.cam.error_detail');
			return $this->setResponseFormat("json")->respond($response, 201);
		}
	}
	...
	public function editEmployee()
	{
		...
		if ($this->validate($rules, $messages)) {
			...
			if ($employee) {
				...
				$cam = $modelEmployee->getEmployeeByValidation($filter_data);
				if ($cam) {
					$response['status'] = 'error';

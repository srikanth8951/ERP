...
class Profile extends ResourceController
{
	...
	{
		$response = array();
		$this->validatePermission('view_asd_Head');	// Check permission
		...
		if ($userDetails) {
			...
			$response['message'] = lang('Employee.asd.success_detail');
			$response['employee'] = [
				'type' => $this->empType,
				'data' => $userDetails
			];
			...
			$response['message'] = lang('Employee.asd.error_detail');
			return $this->setResponseFormat("json")->respond($response, 201);
		}
	}
	...
	public function editEmployee()
	{
		...
		$this->validatePermission('edit_asd');	// Check permission
		...
		if ($this->validate($rules, $messages)) {
			...
			if ($employee) {
				...
				$asd = $modelEmployee->getEmployeeByValidation($filter_data);
				if ($asd) {
					$response['status'] = 'error';

...
class Profile extends ResourceController
{
	...
	{
		$response = array();
		$this->validatePermission('view_dmt_Head');	// Check permission
		...
		if ($userDetails) {
			...
			$response['message'] = lang('Employee.dmt.success_detail');
			$response['employee'] = [
				'type' => $this->empType,
				'data' => $userDetails
			];
			...
			$response['message'] = lang('Employee.dmt.error_detail');
			return $this->setResponseFormat("json")->respond($response, 201);
		}
	}
	...
	public function editEmployee()
	{
		...
		$this->validatePermission('edit_dmt');	// Check permission
		...
		if ($this->validate($rules, $messages)) {
			...
			if ($employee) {
				...
				$dmt = $modelEmployee->getEmployeeByValidation($filter_data);
				if ($dmt) {
					$response['status'] = 'error';

...
class StoreProductRequest extends ResourceController
{
    ...
    public function addProductRequests()
    {
        ...
        $add = $modelStoreRequest->addRequest($product_data);
        if ($add) {
            $product = $modelStoreRequest->addProductRequest($add, $request_products);
            if ($product) {
                $response['status'] = 'success';
    ...
    public function editProductRequests()
    {
        ...
        if ($this->validate($rules, $messages)) {
            ...
            $product_request = $modelStoreRequest->getProductRequests($product_id);
            if ($product_request) {
                $product_data = $this->request->getPost(null);
                $product_data['created_by'] = AuthUser::getId();
                // print_r($product_id, $product_data);
                $edit = $modelStoreRequest->editProductRequests($product_id, $product_data);
                if ($edit) {
                    $response['status'] = 'success';
    ...
    public function deleteProductRequests()
    {
        ...
        $product_request = $modelStoreRequest->getProductRequests($product_id);
        if ($product_request) {
            $remove = $modelStoreRequest->removeProductRequests($product_id);
            if ($remove) {
                $response['status'] = 'success';
    ...
    public function productRequestCancel()
    {
        $response = array();
        ...
        if ($request) {
            ...
            $approval = $modelStoreRequest->addRequestReject($request_id, $approval_data);
            if ($approval) {
                $modelStoreRequest->updateRequest($request_id, ['status' => 2]);
                $response['status'] = 'success';

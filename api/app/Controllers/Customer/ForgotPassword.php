...
class ForgotPassword extends ResourceController
{
			...
			if ($user) {
				if ((int)$user->status == 1) {
					...
					if ($recoverResult) {
						...
						$emailuser = new \App\Controllers\Email\CustomerForgotPassword();
						...
						if ($mailResponse) {
							$response = array(
								'status' => 'success',
								'message' => lang('ForgotPassword.success_password_mail'),
								// 'message_info' => sprintf(lang('Customer.ForgotPassword.success_send_mail'), $user->email)
							);
						} else {
							$response = array(
								'status' => 'error',
								'message' => lang('ForgotPassword.error_update_recovery')
							);
						}
					} else {
						$response = array(
							'status' => 'error',
							'message' => lang('ForgotPassword.error_send_mail')
						);
					}
				} else {
					$response = array(
						'status' => 'error',
						'message' => lang('ForgotPassword.error_account_status')
					);
				}
			} else {
				$response = array(
					'status' => 'error',
					'message' => lang('ForgotPassword.error_no_account')
				);
			}
		} else {
        ...
        if ($user) {
        	if ((int)$user->status == 1) {
	            ...
	            if ($current_date <= $activation_date) {
	                $response = array(
						'status' => 'success',
						'message' => lang('ForgotPassword.success_user_validation'),
						'user' => array(
							'email' => $user->email,
							'mobile' => $user->mobile
						)
	                ...
	                $response = array(
						'status' => 'error',
						'message' => lang('ForgotPassword.error_recovery_time_expired')
					);
					return $this->setResponseFormat("json")->respond($response, 201);
	            }
	        } else {
            ...
            $response = array(
				'status' => 'error',
				'message' => lang('ForgotPassword.error_recovery_link_expired')
			);
			return $this->setResponseFormat("json")->respond($response, 201);
        }
	            ...
	            if ($current_date <= $recover_date) {
					...
					if ($updated) {
		                $verify = $modelUser->resetRecoverEmailToken($user->user_id);
		          		$response = array(
								'status' => 'success',
								'message' => lang('ForgotPassword.success_password_reset')
							);
							return $this->setResponseFormat("json")->respond($response, 200);
					} else {
						$response = array(
							'status' => 'error',
							'message' => lang('ForgotPassword.error_password_reset')
						);
						return $this->setResponseFormat("json")->respond($response, 201);
					}
	                ...
	                $response = array(
						'status' => 'error',
						'message' => lang('ForgotPassword.error_recovery_time_exceeds')
					);
					return $this->setResponseFormat("json")->respond($response, 201);
	            }
	        } else {
	...
	protected function validateForgotForm()
	{
        ...
        if (!filter_var($user_email, FILTER_VALIDATE_EMAIL)) {
            $this->error['email'] = lang('ForgotPassword.error_user_email');
        }

...
class RegionHead extends ResourceController
{
	...
	{
		$response = array();
		$this->validatePermission('add_regionHead');	// Check permission
		...
		if ($this->validate($rules, $messages)) {
			...
			$regionHead_data = array_merge($this->request->getPost(null), [
				'user_type' => $user_type['type_id'],
				'is_exist' => 0
			]);

			$val_filter_data = array(
				'removed' => 0,
				'status' => 1,
				'username' => $regionHead_data['username'],
				'email' => $regionHead_data['email']
			);
			$regionHead = $modelEmployee->getEmployeeByValidation($val_filter_data);
			...
			if ($regionHead) {
				...
				$filter_data = array(
					...
					'status' => 1,
					'user_type' => $user_type['type_id'],
					'region_id' => $regionHead_data['region_id']
				);
				...
				if ($region_id) {
					...
					$add = $modelEmployee->addEmployee($regionHead_data);
					if ($add) {
						$response['status'] = 'success';
						$response['message'] = lang('Employee.RegionHead.success_add');
						return $this->setResponseFormat("json")->respond($response, 200);
					} else {
						$response['status'] = 'error';
						$response['message'] = lang('Employee.RegionHead.error_add');
						return $this->setResponseFormat("json")->respond($response, 201);
					}
				}
			}
	...
	public function editEmployee()
	{
		...
		$this->validatePermission('edit_regionHead');	// Check permission
		$user_type = getUserTypeByCode($this->empType);
		...
		if ($this->validate($rules, $messages)) {
			...
			$regionHead_detail = $modelEmployee->getEmployee($employee_id);
			if ($regionHead_detail) {
				$val_filter_data = array(
					'removed' => 0,
					'status' => 1,
					'username' => $regionHead_data['username'],
					'email' => $regionHead_data['email'],
					'except' => [$regionHead_detail->user_id]
				);
				$regionHead = $modelEmployee->getEmployeeByValidation($val_filter_data);
				if ($regionHead) {
					$response['status'] = 'error';
					...
					if ($region_id) {
						...
						$edit = $modelEmployee->editEmployee($employee_id, $regionHead_data);
						if ($edit) {
							$response['status'] = 'success';
							$response['message'] = lang('Employee.RegionHead.success_edit');
							return $this->setResponseFormat("json")->respond($response, 200);
						} else {
							$response['status'] = 'error';
							$response['message'] = lang('Employee.RegionHead.error_edit');
							return $this->setResponseFormat("json")->respond($response, 201);
						}
					}
				}
	...
	public function deleteEmployee()
	{
		...
		$regionHead = $modelEmployee->getEmployee($employee_id);
		if ($regionHead) {
			$remove = $modelEmployee->removeEmployee($employee_id, $regionHead->user_id);
			if ($remove) {
				$response['status'] = 'success';
				$response['message'] = lang('Employee.RegionHead.success_removed');
				return $this->setResponseFormat("json")->respond($response, 200);
			} else {
				$response['status'] = 'error';
				$response['message'] = lang('Employee.RegionHead.error_removed');
				return $this->setResponseFormat("json")->respond($response, 201);
			}
		} else {
			$response['status'] = 'error';
	...
	public function setEmployeeStatus()
	{
		...
		$regionHead = $modelEmployee->getEmployee($employee_id);
		if ($regionHead) {
			$remove = $modelEmployee->setEmployeeStatus($employee_id, $status, $regionHead->user_id);
			if ($remove) {
				$response['status'] = 'success';
	...
	public function autocomplete()
	{
		...
		$regionHeadArray = array();
		$regionHeads = $modelEmployee->getEmployees($filter_data);
		if ($regionHeads) {
			foreach ($regionHeads as $regionHead) {
				$regionHeadArray[] = array(
					'id' => (int)$regionHead->regionHead_id,
					'name' => html_entity_decode($regionHead->first_name . '' . $regionHead->last_name),
				);
			}
		}
		$response = array(
	...
	public function downloadSample()
	{
		...
		$response = array(
			'status' => 'success',
			'message' => lang('Employee.RegionHead..success_sample_download'),
	...
	public function createExcel()
	{
		...
		$fileName = 'region-' . date('YmdHis') . '.xlsx';

...
class AreaServiceDeliveryHead extends ResourceController
{
	...
	public function index()
	{
		...
		$this->validatePermission('view_ASDHead');	// Check permission
		...
		$user_type = getUserTypeByCode('asd_head');
		...
		$total_ASD_heads	= $modelEmployee->getTotalEmployees($filter_data);
		$ASD_heads	= $modelEmployee->getEmployees($filter_data);

		if ($ASD_heads) {
			$response = array(
				'status' => 'success',
				'message' => lang('Employee.ASDHead.success_list'),
				'employees' => [
					'type' => 'ASD_head',
					'data' => $ASD_heads,
					'pagination' => array(
						'total' => (int)$total_ASD_heads,
						'length' => $limit,
						'start' => $start,
						'records' => count($ASD_heads)
					)
				]
			);
			...
			$response = array(
				'status' => 'error',
				'message' => lang('Employee.ASDHead.error_list')
			);
	...
	public function getEmployee()
	{
		...
		$ASDHead = $modelEmployee->getEmployee($employee_id);
		if ($ASDHead) {
			$response['status'] = 'success';
			$response['message'] = lang('Employee.ASDHead.success_detail');
			$response['employee'] = [
				'type' => $this->empType,
				'data' => $ASDHead
			];
			...
			$response['message'] = lang('Employee.ASDHead.error_detail');
			return $this->setResponseFormat("json")->respond($response, 201);
		}
	}
	...
	{
		$response = array();
		$this->validatePermission('add_ASDHead');	// Check permission
		...
		if ($this->validate($rules, $messages)) {
			...
			$ASDHead_data = array_merge($this->request->getPost(null), [
				'user_type' => $user_type['type_id'],
				'is_exist' => 0
			]);
			...
			$filter_data = array(
				'removed' => 0,
				'status' => 1,
				'username' => $ASDHead_data['username'],
				'email' => $ASDHead_data['email']
			);

			$ASDHead = $modelEmployee->getEmployeeByValidation($filter_data);
			if ($ASDHead) {
				$response['status'] = 'error';
				$response['message'] = lang('Employee.ASDHead.error_exist');
				return $this->setResponseFormat("json")->respond($response, 201);
			} else {
				$filter_data = array(
					'removed' => 0,
					'status' => 1,
					'user_type' => $user_type['type_id'],
					'region_id' => $ASDHead_data['region_id'],
					'branch_id' => $ASDHead_data['branch_id'],
				);
				$branch_id = $modelEmployee->getEmployees($filter_data);
				if ($branch_id) {
					$response['status'] = 'error';
					$response['message'] = lang('Employee.AreaHead.error_exist');
					return $this->setResponseFormat("json")->respond($response, 201);
				} else {
					$add = $modelEmployee->addEmployee($ASDHead_data);
					if ($add) {
						$response['status'] = 'success';
						$response['message'] = lang('Employee.ASDHead.success_add');
						return $this->setResponseFormat("json")->respond($response, 200);
					} else {
						$response['status'] = 'error';
						$response['message'] = lang('Employee.ASDHead.error_add');
						return $this->setResponseFormat("json")->respond($response, 201);
					}
				}
			}
	...
	public function editEmployee()
	{
		...
		$this->validatePermission('edit_ASDHead');	// Check permission
		$user_type = getUserTypeByCode('asd_head');
		...
		if ($this->validate($rules, $messages)) {
			...
			$ASDHead_detail = $modelEmployee->getEmployee($employee_id);
			if ($ASDHead_detail) {
				$filter_data = array(
					'removed' => 0,
					'status' => 1,
				...
				if ($ASDHead) {
					...
					if ($branch_id) {
						...
						$edit = $modelEmployee->editEmployee($employee_id, $ASDHead_data);
						if ($edit) {
							$response['status'] = 'success';
							$response['message'] = lang('Employee.ASDHead.success_edit');
							return $this->setResponseFormat("json")->respond($response, 200);
						} else {
							$response['status'] = 'error';
							$response['message'] = lang('Employee.ASDHead.error_edit');
							return $this->setResponseFormat("json")->respond($response, 201);
						}
					}
				}
	...
	public function deleteEmployee()
	{
		...
		if ($ASDHead) {
			...
			$remove = $modelEmployee->removeEmployee($employee_id, $ASDHead->user_id);

			if ($remove) {
				$response['status'] = 'success';
				$response['message'] = lang('Employee.ASDHead.success_removed');
				return $this->setResponseFormat("json")->respond($response, 200);
			} else {
				$response['status'] = 'error';
				$response['message'] = lang('Employee.ASDHead.error_removed');
				return $this->setResponseFormat("json")->respond($response, 201);
			}
		} else {
			$response['status'] = 'error';
	...
	public function setEmployeeStatus()
	{
		...
		$ASDHead = $modelEmployee->getEmployee($employee_id);
		if ($ASDHead) {
			$remove = $modelEmployee->setEmployeeStatus($employee_id, $status, $ASDHead->user_id);
			if ($remove) {
				$response['status'] = 'success';
	...
	public function autocomplete()
	{
		...
		$ASDHeadArray = array();
		$ASDHeads = $modelEmployee->getEmployees($filter_data);
		if ($ASDHeads) {
			foreach ($ASDHeads as $ASDHead) {
				$ASDHeadArray[] = array(
					'id' => (int)$ASDHead->ASDHead_id,
					'name' => html_entity_decode($ASDHead->first_name . '' . $ASDHead->last_name)
				);
			}
		}
		$response = array(
	...
	public function downloadSample()
	{
		...
		$response = array(
			'status' => 'success',
			'message' => lang('Employee.ASDHead.success_sample_download'),
	...
	public function createExcel()
	{
		...
		$sheet->setCellValue('P1', 'Designation');
		$sheet->setCellValue('Q1', 'Work Expertise');
		$sheet->setCellValue('R1', 'Date of Joining');
		...
		for ($x = 2; $x < 30; $x++) {
			...
			$validation = $sheet->getCell('O' . $x)->getDataValidation();
			...
			$validation1 = $sheet->getCell('P' . $x)->getDataValidation();
			...
			$validation4 = $sheet->getCell('Q' . $x)->getDataValidation();
		...
		if (!$input) {
			...
			if ($upload['status'] == 'success') {
				...
				for ($i = 2; $i <= $arrayCount; $i++) {
					...
					$dept 	 =  explode('-', $allDataInSheet[$i]["O"]);
					$degn    =  explode('-', $allDataInSheet[$i]["P"]);
					$work    =  explode('-', $allDataInSheet[$i]["Q"]);
					...
					$EmployeeArr[$i]["joining_date"] = $allDataInSheet[$i]["R"] ?? 0;

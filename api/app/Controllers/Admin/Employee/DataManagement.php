...
class DataManagement extends ResourceController
{
	...
	public function index()
	{
		...
		$this->validatePermission('view_DataManagement');	// Check permission
		...
		if ($region_heads) {
			$response = array(
				'status' => 'success',
				'message' => lang('Employee.DataManagement.success_list'),
				'employees' => [
					'type' => $this->empType,
					'data' => $region_heads,
					'pagination' => array(
			...
			$response = array(
				'status' => 'error',
				'message' => lang('Employee.DataManagement.error_list')
			);
	...
	public function getEmployee()
	{
		...
		$DataManagement = $modelEmployee->getEmployee($employee_id);
		if ($DataManagement) {
			$response['status'] = 'success';
			$response['message'] = lang('Employee.DataManagement.success_detail');
			$response['employee'] = [
				'type' => $this->empType,
				'data' => $DataManagement
			];
			...
			$response['message'] = lang('Employee.DataManagement.error_detail');
			return $this->setResponseFormat("json")->respond($response, 201);
		}
	}
	...
	{
		$response = array();
		$this->validatePermission('add_DataManagement');	// Check permission
		...
		if ($this->validate($rules, $messages)) {
			...
			$DataManagement_data = array_merge($this->request->getPost(null), [
				'user_type' => $user_type['type_id'],
				'is_exist' => 0
			]);
			...
			$filter_data = array(
				'removed' => 0,
				'status' => 1,
				'username' => $DataManagement_data['username'],
				'email' => $DataManagement_data['email']
			);

			$DataManagement = $modelEmployee->getEmployeeByValidation($filter_data);
			if ($DataManagement) {
				$response['status'] = 'error';
				$response['message'] = lang('Employee.DataManagement.error_exist');
				return $this->setResponseFormat("json")->respond($response, 201);
			} else {
				$add = $modelEmployee->addEmployee($DataManagement_data);
				if ($add) {
					$response['status'] = 'success';
					$response['message'] = lang('Employee.DataManagement.success_add');
					return $this->setResponseFormat("json")->respond($response, 200);
				} else {
					$response['status'] = 'error';
					$response['message'] = lang('Employee.DataManagement.error_add');
					return $this->setResponseFormat("json")->respond($response, 201);
				}
			}
		} else {
	...
	public function editEmployee()
	{
		...
		$this->validatePermission('edit_DataManagement');	// Check permission
		$user_type = getUserTypeByCode($this->empType);
		...
		if ($this->validate($rules, $messages)) {
			...
			$DataManagement_detail = $modelEmployee->getEmployee($employee_id);
			if ($DataManagement_detail) {
				$filter_data = array(
					'removed' => 0,
					'status' => 1,
					'username' => $DataManagement_data['username'],
					'email' => $DataManagement_data['email'],
					'except' => [$DataManagement_detail->user_id]
				);
				...
				if ($DataManagement) {
					...
					$edit = $modelEmployee->editEmployee($employee_id, $DataManagement_data);
					if ($edit) {
						$response['status'] = 'success';
						$response['message'] = lang('Employee.DataManagement.success_edit');
						return $this->setResponseFormat("json")->respond($response, 200);
					} else {
						$response['status'] = 'error';
						$response['message'] = lang('Employee.DataManagement.error_edit');
						return $this->setResponseFormat("json")->respond($response, 201);
					}
				}
			} else {
	...
	public function deleteEmployee()
	{
		...
		if ($DataManagement) {
			...
			$remove = $modelEmployee->removeEmployee($employee_id, $DataManagement->user_id);

			if ($remove) {
				$response['status'] = 'success';
				$response['message'] = lang('Employee.DataManagement.success_removed');
				return $this->setResponseFormat("json")->respond($response, 200);
			} else {
				$response['status'] = 'error';
				$response['message'] = lang('Employee.DataManagement.error_removed');
				return $this->setResponseFormat("json")->respond($response, 201);
			}
		} else {
			$response['status'] = 'error';
	...
	public function setEmployeeStatus()
	{
		...
		$DataManagement = $modelEmployee->getEmployee($employee_id);
		if ($DataManagement) {
			$remove = $modelEmployee->setEmployeeStatus($employee_id, $status, $DataManagement->user_id);
			if ($remove) {
				$response['status'] = 'success';
	...
	public function autocomplete()
	{
		...
		$DataManagementArray = array();
		$DataManagements = $modelEmployee->getEmployees($filter_data);
		if ($DataManagements) {
			foreach ($DataManagements as $DataManagement) {
				$DataManagementArray[] = array(
					'id' => (int)$DataManagement->DataManagement_id,
					'name' => html_entity_decode($DataManagement->first_name . '' . $DataManagement->last_name)
				);
			}
		}
		$response = array(
	...
	public function downloadSample()
	{
		...
		$response = array(
			'status' => 'success',
			'message' => lang('Employee.DataManagement.success_sample_download'),
	...
	public function createExcel()
	{
		...
		$fileName = 'dataManagement-' . date('YmdHis') . '.xlsx';

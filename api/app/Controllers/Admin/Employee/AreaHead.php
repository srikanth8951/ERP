...
class AreaHead extends ResourceController
{
	...
	{
		$response = array();
		$this->validatePermission('add_areaHead');	// Check permission
		...
		if ($this->validate($rules, $messages)) {
			...
			$areaHead_data = array_merge($this->request->getPost(null), [
				'user_type' => $user_type['type_id'],
				'is_exist' => 0
			]);
			...
			$filter_data = array(
				'removed' => 0,
				'status' => 1,
				'username' => $areaHead_data['username'],
				'email' => $areaHead_data['email']
			);

			$areaHead = $modelEmployee->getEmployeeByValidation($filter_data);
			if ($areaHead) {
				$response['status'] = 'error';
				...
				$filter_data = array(
					...
					'status' => 1,
					'user_type' => $user_type['type_id'],
					'region_id' => $areaHead_data['region_id'],
					'branch_id' => $areaHead_data['branch_id'],
				);
				...
				if ($branch_id) {
					...
					$add = $modelEmployee->addEmployee($areaHead_data);
					if ($add) {
						$response['status'] = 'success';
						$response['message'] = lang('Employee.AreaHead.success_add');
						return $this->setResponseFormat("json")->respond($response, 200);
					} else {
						$response['status'] = 'error';
						$response['message'] = lang('Employee.AreaHead.error_add');
						return $this->setResponseFormat("json")->respond($response, 201);
					}
				}
			}
	...
	public function editEmployee()
	{
		...
		$this->validatePermission('edit_areaHead');	// Check permission
		$user_type = getUserTypeByCode($this->empType);
		...
		if ($this->validate($rules, $messages)) {
			...
			$areaHead_detail = $modelEmployee->getEmployee($employee_id);
			// print_r($areaHead_detail);
			if ($areaHead_detail) {
				$filter_data = array(
					'removed' => 0,
					'status' => 1,
					'username' => $areaHead_data['username'],
					'email' => $areaHead_data['email'],
					'except' => [$areaHead_detail->user_id]
				);
				...
				if ($areaHead) {
					...
					if ($branch_id) {
						...
						$edit = $modelEmployee->editEmployee($employee_id, $areaHead_data);
						if ($edit) {
							$response['status'] = 'success';
							$response['message'] = lang('Employee.AreaHead.success_edit');
							return $this->setResponseFormat("json")->respond($response, 200);
						} else {
							$response['status'] = 'error';
							$response['message'] = lang('Employee.AreaHead.error_edit');
							return $this->setResponseFormat("json")->respond($response, 201);
						}
					}
				}
	...
	public function deleteEmployee()
	{
		...
		$employee_id = $this->request->getVar('employee_id');
		$areaHead = $modelEmployee->getEmployee($employee_id);
		if ($areaHead) {
			//$remove = $modelEmployee->deleteEmployee($employee_id);
			$remove = $modelEmployee->removeEmployee($employee_id, $areaHead->user_id);

			if ($remove) {
				$response['status'] = 'success';
				$response['message'] = lang('Employee.AreaHead.success_removed');
				return $this->setResponseFormat("json")->respond($response, 200);
			} else {
				$response['status'] = 'error';
				$response['message'] = lang('Employee.AreaHead.error_removed');
				return $this->setResponseFormat("json")->respond($response, 201);
			}
		} else {
			$response['status'] = 'error';
	...
	public function setEmployeeStatus()
	{
		...
		$areaHead = $modelEmployee->getEmployee($employee_id);
		if ($areaHead) {
			$remove = $modelEmployee->setEmployeeStatus($employee_id, $status, $areaHead->user_id);
			if ($remove) {
				$response['status'] = 'success';
				$response['message'] = lang('Employee.AreaHead.success_status');
				return $this->setResponseFormat("json")->respond($response, 200);
			} else {
				$response['status'] = 'error';
				$response['message'] = lang('Employee.AreaHead.error_removed');
	...
	public function autocomplete()
	{
		...
		if ($areaHeads) {
			foreach ($areaHeads as $areaHead) {
				$areaHeadArray[] = array(
					'id' => (int)$areaHead->areaHead_id,
					'name' => html_entity_decode($areaHead->first_name . '' . $areaHead->last_name)
				);
			}
		}
		$response = array(
	...
	public function downloadSample()
	{
		...
		$response = array(
			'status' => 'success',
			'message' => lang('Employee.AreaHead.success_sample_download'),

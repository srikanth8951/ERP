...
class AllIndiaServiceDeliveryHead extends ResourceController
{
	...
	public function index()
	{
		...
		$user_type = getUserTypeByCode('aisd_head');
	...
	{
		$response = array();
		$this->validatePermission('add_AISDHead');	// Check permission
		...
		if ($this->validate($rules, $messages)) {
			...
			$AISDHead_data = array_merge($this->request->getPost(null), [
				'region_id'	=> 0,
				'area_id' => 0,
				'is_exist' => 0,
				'user_type' => $user_type['type_id']
			...
			$filter_data = array(
				'removed' => 0,
				'status' => 1,
				'username' => $AISDHead_data['username'],
				'email' => $AISDHead_data['email']
			);
			$AISDHead = $modelEmployee->getEmployeeByValidation($filter_data);
			if ($AISDHead) {
				$response['status'] = 'error';
				$response['message'] = lang('Employee.AISDHead.error_exist');
				return $this->setResponseFormat("json")->respond($response, 201);
			} else {
				$add = $modelEmployee->addEmployee($AISDHead_data);
				if ($add) {
					$response['status'] = 'success';
					$response['message'] = lang('Employee.AISDHead.success_add');
					return $this->setResponseFormat("json")->respond($response, 200);
				} else {
					$response['status'] = 'error';
					$response['message'] = lang('Employee.AISDHead.error_add');
					return $this->setResponseFormat("json")->respond($response, 201);
				}
			}
		} else {
	...
	public function editEmployee()
	{
		...
		if ($this->validate($rules, $messages)) {
			...
			$AISDHead_detail = $modelEmployee->getEmployee($employee_id);
			if ($AISDHead_detail) {
				$filter_data = array(
					'removed' => 0,
					'status' => 1,
					'username' => $AISDHead_data['username'],
					'email' => $AISDHead_data['email'],
					'except' => [$AISDHead_detail->user_id]
				);
				$AISDHead = $modelEmployee->getEmployeeByValidation($filter_data);
				if ($AISDHead) {
					$response['status'] = 'error';
					...
					$edit = $modelEmployee->editEmployee($employee_id, $AISDHead_data);
					if ($edit) {
						$response['status'] = 'success';
						$response['message'] = lang('Employee.AISDHead.success_edit');
						return $this->setResponseFormat("json")->respond($response, 200);
					} else {
						$response['status'] = 'error';
						$response['message'] = lang('Employee.AISDHead.error_edit');
						return $this->setResponseFormat("json")->respond($response, 201);
					}
				}
			} else {
	...
	public function deleteEmployee()
	{
		...
		if ($AISDHead) {
			...
			$remove = $modelEmployee->removeEmployee($employee_id, $AISDHead->user_id);

			if ($remove) {
				$response['status'] = 'success';
				$response['message'] = lang('Employee.AISDHead.success_removed');
				return $this->setResponseFormat("json")->respond($response, 200);
			} else {
				$response['status'] = 'error';
	...
	public function autocomplete()
	{
		...
		$AISDHeadArray = array();
		$AISDHeads = $modelEmployee->getEmployees($filter_data);
		if ($AISDHeads) {
			foreach ($AISDHeads as $AISDHead) {
				$AISDHeadArray[] = array(
					'id' => (int)$AISDHead->AISDHead_id,
					'name' => html_entity_decode($AISDHead->first_name . '' . $AISDHead->last_name),
				);
			}
		}
		$response = array(
	...
	public function downloadSample()
	{
		...
		$response = array(
			'status' => 'success',
			'message' => lang('Employee.AISDHead.success_sample_download'),
	...
	public function createExcel()
	{
		...
		$sheet->setCellValue('N1', 'Designation');
		$sheet->setCellValue('O1', 'Work Expertise');
		...
		for ($x = 2; $x < 30; $x++) {
			...
			$validation1 = $sheet->getCell('N' . $x)->getDataValidation();
			...
			$validation4 = $sheet->getCell('O' . $x)->getDataValidation();
		...
		$fileName = 'aisd-' . date('YmdHis') . '.xlsx';
		...
		if (!$input) {
			...
			if ($upload['status'] == 'success') {
				...
				for ($i = 2; $i <= $arrayCount; $i++) {
					...
					$dept 	 =  explode('-', $allDataInSheet[$i]["M"]);
					$degn    =  explode('-', $allDataInSheet[$i]["N"]);
					$work    =  explode('-', $allDataInSheet[$i]["O"]);
					$user_type = getUserTypeByCode('aisd_head');
					...
					$EmployeeArr[$i]["joining_date"] = $allDataInSheet[$i]["P"] ?? 0;

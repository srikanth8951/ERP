<?php
    ...
    $oAndM = getContractTypeByCode('oandm');
    $except[] = $oAndM['id'] ?? 0;
    $contractTypes = getContractTypes($except);
    $contractTypeIdss = [];
    if ($contractTypes) {
        foreach ($contractTypes as $contractType) {
            $contractTypeIdss[] = $contractType['id'];
        }
    }
    $contractTypeIds = implode(',', $contractTypeIdss);
?>
        ...
        <div class="p-2" style="background-color: #dee1e5;"> 
            <div class="row">
                ...
                <div class="col text-right">
                    <a href="<?php echo base_url('admin/contract_job/add') ?>" class="btn  btn-outline-primary waves-effect waves-light ">Add Contract</a>
                </div>
            </div>
        </div>
        <br>
...
<script>
    window.contract_type_id = '<?php echo $contractTypeIds; ?>';
</script>
...
<script src="<?php echo base_url('assets/js/include/admin/contract_job_list.js'); ?>"></script>
...
<script>
    $(function(){
        ...
        function loadInitFunctions() {
            loadContractJobDetails(formApiUrl('admin/contract_job/list'));
        }

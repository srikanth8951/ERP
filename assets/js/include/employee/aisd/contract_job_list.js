 ...
 */
 ...
 $(function () {
                     ...
                     if (res.contract_jobs) {
                         ...
                         if (details.length && pagination.total > 0) {
                             ...
                             $.each(details, function (listIn, listVal) {
                                 ...
                                 var contractJobViewLink = formUrl('employee/aisd/contract_job/view/' + listVal.contract_job_id);
                                ...
                                //      if(expiry_days < 1) {
                                //          contractJobRenewLink = formUrl('employee/aisd/renew/' + listVal.contract_job_id);
                                ...
                                //          contractJobUpdateLink = formUrl('employee/aisd/update/' + listVal.contract_job_id);
                             ...
                             listPagination.find(".list-pagination").pagination({
                                 ...
                                 onPageClick: function (pageNumber, event) {
                                     var page_link = formApiUrl('employee/aisd/contract_job/list', { start: ((parseInt(pagination.length) * (pageNumber - 1)) + 1)});
     ...
     searchForm.submit(function (e) {
         e.preventDefault();
         loadContractJobDetails(formApiUrl('employee/aisd/contract_job/'));  // Load contract job details
     });
     ...
     $(listContainer).on('click', '.btn-switch-contractjob-status', function (e) {
         ...
         Swal.fire({
             ...
             if (result.isConfirmed) {
                 ...
                 $.ajax({
                     url: formApiUrl('employee/aisd/status/update', { contract_job_id: contractjob_id }),
                     type: 'post',
                     dataType: 'json',
                     data: { status: status },
                     headers: {
                     ...
                     if (res.status == 'success') {
                         loadContractJobDetails(formApiUrl('employee/aisd/contract_job/'));  // Load employee details

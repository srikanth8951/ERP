 ...
 */
 ...
 $(function () {
                     ...
                     if (res.product_requests) {
                         ...
                         if (details.length && pagination.total > 0) {
                             $.each(details, function (listIn, listVal) {
                                if (listVal.status.id == 7) {
                                    actionBtnIssue = '<button class="text-white btn btn-sm btn-success waves-effect waves-light mr-1" title="Issue" id="btn-request-issue" data-request-id="' + listVal.request_id + '"><i class="mdi mdi-check"></i></button>';
                                } else {
                                    actionBtnIssue = '';
                                }
                                 listContainer.append('<tr>' +
                                     ...
                                     '<td>' +
                                     '<a href="' + formUrl('employee/store/requests/view/' + listVal.request_id) + '" class="text-white btn btn-sm btn-danger waves-effect waves-light mr-1" data-toggle="tooltip" data-placement="top" title="View"><i class="mdi mdi-eye"></i></a>' +
                                     actionBtnIssue +
                                     '</td>' +
                                     '</tr>');
                             });
                             ...
                             listPagination.find('.list-pagination').pagination({
                                 ...
                                 onPageClick: function (pageNumber, event) {
                                     var page_link = formApiUrl('employee/store/product/request/list', { start: ((parseInt(pagination.length) * (pageNumber - 1)) + 1)});
     ...
     lengthContainer.change(function () {
         loadDetails(formApiUrl('employee/store/product/request/list'));  // Load request details
     });
     ...
     //Request Approval
    $(listContainer).on('click', "#btn-request-issue", function (e) {
        e.preventDefault();
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    url: formApiUrl('employee/store/product/request/approval', { request_id: request_id }),
                    type: 'post',
                    dataType: 'json',
                    data: { status: status },
                    headers: {
                    ...
                    if (res.status == 'success') {
                        loadDetails(formApiUrl('employee/store/product/request/list'));  // Load employee details

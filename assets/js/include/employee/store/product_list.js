...
$(function () {
          ...
          if (res.products) {
            ...
            if (details.length && pagination.total > 0) {
              ...
              $.each(details, function (listIn, listVal) {
                ...
                let url = formUrl(
                  "employee/store/products/edit/" + listVal.product_id
                );
                listContainer.append(
                  "<tr>" +
                    "<td>" +
              ...
              listPagination.find(".list-pagination").pagination({
                ...
                onPageClick: function (pageNumber, event) {
                  var page_link = formApiUrl("employee/store/product/list", {
  ...
  lengthContainer.change(function () {
    loadDetails(formApiUrl("employee/store/product/list")); // Load product details
  });
  ...
  $(listContainer).on("click", ".btn-delete-product", function (e) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        $.ajax({
          url: formApiUrl("employee/store/product/delete", {
  ...
  $(listContainer).on("click", "#btn-update-quantity", function (e) {
    ...
    quantityModal.find(quantityForm).attr(
      "action",
      formApiUrl("employee/store/product/add/stock", {
        product_id: product_id,
      })
    );
    ...
    $.ajax({
      ...
      .done(function (res) {
        if (res.status == "success") {
          ...
          setTimeout(function () {
            window.location.href = formUrl("employee/store/products");
          });
        } else if (res.status == "error") {
          if (

...
$(function () {
    ...
    if (parseValue(detail) != "" && Object.keys(detail).length > 0) {
      ...
      if (detail.products) {
        ...
        $.each(detail.products, function (listIn, listVal) {
          ...
          let stock_status = ''
          let stock_tip = ''
          if (parseInt(listVal.quantity) >= parseInt(listVal.requested_quantity)) {
            stock_status = 'text-success'
            stock_tip = 'In stock'
          } else {
            stock_status = 'text-danger'
            stock_tip = 'Out of stock'
          }
          ...
          <tbody>
            <tr>
                ...
                <td><span data-toggle="tooltip" title="${stock_tip}"> ${listVal.requested_quantity} <small class="${stock_status}">/ ${ listVal.quantity }</small></span></td>
                <td>${total_price}</td>
            </tr>
          </tbody>`);
        });
      ...
      requestbDetailArea.find('[data-toggle="tooltip"]').tooltip();
    } else {
  ...
  window.loadRequestDetail = function () {
    ...
    $.ajax({
      url: formApiUrl("employee/store/product/request/detail", {

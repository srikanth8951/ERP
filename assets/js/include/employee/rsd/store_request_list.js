 ...
 */
 ...
 $(function () {
                         ...
                         if (details.length && pagination.total > 0) {
                             ...
                             $.each(details, function (listIn, listVal) {
                                if (listVal.status.id == 1 || listVal.status.id == 3 || listVal.status.id == 5 || listVal.status.id == 4 || listVal.status.id == 6) {
                                 ...
                                 let url = formUrl('employee/rsd/store_request/edit/' + listVal.request_id)
                                 listContainer.append('<tr>' +
                                     ...
                                     '<td>' +
                                     '<a href="' + formUrl('employee/rsd/store_request/view/' + listVal.request_id) + '" class="text-white btn btn-sm btn-info waves-effect waves-light mr-1" data-toggle="tooltip" data-placement="top" title=""><i class="mdi mdi-eye"></i></a>' +
                                     actionBtnApprove +
                                     actionBtnReject +
                                     '</td>' +
                                     '</tr>');
                             ...
                             listPagination.find('.list-pagination').pagination({
                                 ...
                                 onPageClick: function (pageNumber, event) {
                                     var page_link = formApiUrl('employee/rsd/store_product/request/list', { start: ((parseInt(pagination.length) * (pageNumber - 1)) + 1)});
     ...
     lengthContainer.change(function () {
         loadDetails(formApiUrl('employee/rsd/store_product/request/list'));  // Load request details
     });
    ...
    $(listContainer).on('click', "#btn-request-approval", function (e) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    url: formApiUrl('employee/rsd/store_product/request/approval', { request_id: request_id }),
                    type: 'post',
                    dataType: 'json',
                    data: { status: status },
                    headers: {
                    ...
                    if (res.status == 'success') {
                        loadDetails(formApiUrl('employee/rsd/store_product/request/list'));  // Load employee details
    ...
    $(listContainer).on('click', '#btn-request-reject', function (e) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    url: formApiUrl('employee/rsd/store_product/request/reject', { request_id: request_id }),
                    type: 'post',
                    dataType: 'json',
                    data: { status: status },
                    headers: {

...
$(function () {
    ...
    function loadAssetChecklistView(element, data) {
        ...
        element.find('.asset-checklist-body .checklists-area').prepend(`<li class="checklist-row d-flex align-items-center justify-content-between border rounded p-2 mt-2">
             <input type="hidden" name="assets[${data.assetCount}][checklist][]" value="${data.id}" />
             <span>${data.name}</span>
    ...
    window.loadAssetView = async function (options) {
                                 ...
                                 <div class="card h-100">
                                     <div class="card-header shadow-sm border-0">
                                         <div class="d-flex align-items-center justify-content-between">
                                             ...
                                             <button type="button" data-toggle="collapse" data-target="#assets${assetCount}-checklist" class="btn btn-sm btn-purple waves-effect waves-light"><i class="mdi mdi-plus"></i>&nbsp;Add</button>
                                         </div>
                                     </div>
                                     <div class="card-body asset-checklist-body">
                                         <div class="collapse p-2 bg-light" id="assets${assetCount}-checklist" style="position: absolute;top: 0;left: 0;right: 0;z-index: 9;">
                                             <div class="d-flex align-items-center w-100 mb-2">
                                                 <select data-toggle="jy-select2-assign-checklist" data-asset-count="${assetCount}" class="select2"></select>
                                                 <span data-toggle="collapse" data-target="#assets${assetCount}-checklist" class="ml-2 font-20 text-danger"><i class="mdi mdi-close-circle"></i></span>
                                             </div>
                                         </div>
        ...
        $('#asset-row' + assetCount + ' [data-toggle="jy-select2-assign-checklist"]').select2({
            ...
            ajax: {
                url: formApiUrl('admin/checklist/autocomplete', { limit: 10 }),
                dataType: 'json',
                type: 'get',
                delay: 250,
                headers: {

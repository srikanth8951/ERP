$(function () {
    ...
    $('#select_engineer').on('select2:select', function (e) {
        let engVal = $(this).val();
           ...
           // Get engineer detail
           $.ajax({
            url: formApiUrl("admin/employee/engineer/getdetails", { engineer_id: engVal }),
            type: "get",
            dataType: "json",
            headers: {
                Authorization: `Bearer ${wapLogin.getToken()}`,
    ...
    function loadEngineerDetails(detail) {
        if (parseValue(detail) != '' && Object.keys(detail).length > 0) {
            console.log(detail);
            contractJobForm.find('.engineer-detail-area input[type="text"]').prop('disabled', true);
            ...
            contractJobForm.find('.engineer-detail-area input[type="text"]').prop('disabled', false);
        }
    }

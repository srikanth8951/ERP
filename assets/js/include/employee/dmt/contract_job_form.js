...
$(function () {
  ...
  function loadContractJobView(detail) {
    if (parseValue(detail) != "" && Object.keys(detail).length > 0) {
      contractJobForm.find('[name="job_title"]').val(detail.job_title).trigger("change");
  ...
  window.loadContractJobDetail = function () {
    ...
    $.ajax({
      url: formApiUrl("employee/dmt/contract_job/detail", {
    ...
    $.ajax({
      ...
      .done(function (res) {
        if (res.status == "success") {
          ...
          setTimeout(function () {
            window.location.href = formUrl("employee/dmt/contract_job");
          });
        } else if (res.status == "error") {
          if (

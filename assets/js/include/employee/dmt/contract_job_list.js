 ...
 */
 ...
 $(function () {
                     ...
                     if (res.contract_jobs) {
                         ...
                         if (details.length && pagination.total > 0) {
                             ...
                             $.each(details, function (listIn, listVal) {
                                 ...
                                 var contractJobViewLink = formUrl('employee/dmt/contract_job/view/' + listVal.contract_job_id);
                                 ...
                                 if(parseValue(listVal.is_parent) == '') {
                                     if(expiry_days < 1) {
                                         contractJobRenewLink = formUrl('employee/dmt/contract_job/renew/' + listVal.contract_job_id);
                                         ...
                                         contractJobStatusOption = `<input type="checkbox" data-contractjob="${listVal.contract_job_id}" id="switch${listVal.contract_job_id}" class="btn-switch-contractjob-status" switch="bool" value="${listVal.removed}" ${status}/>
                                         <label for="switch${listVal.contract_job_id}" class="m-0"></label>`;
 
                                         contractJobUpdateLink = formUrl('employee/dmt/contract_job/update/' + listVal.contract_job_id);
                             ...
                             listPagination.find(".list-pagination").pagination({
                                 ...
                                 onPageClick: function (pageNumber, event) {
                                     var page_link = formApiUrl('employee/dmt/contract_job/list', { start: ((parseInt(pagination.length) * (pageNumber - 1)) + 1)});
     ...
     searchForm.submit(function (e) {
         e.preventDefault();
         loadContractJobDetails(formApiUrl('employee/dmt/contract_job/list'));  // Load contract job details
     });
     ...
     $(listContainer).on('click', '.btn-switch-contractjob-status', function (e) {
         ...
         Swal.fire({
             ...
             if (result.isConfirmed) {
                 ...
                 $.ajax({
                     url: formApiUrl('employee/dmt/contract_job/status/update', { contract_job_id: contractjob_id }),
                     type: 'post',
                     dataType: 'json',
                     data: { status: status },
                     headers: {
                     ...
                     if (res.status == 'success') {
                         loadContractJobDetails(formApiUrl('employee/dmt/contract_job/list'));  // Load employee details
     ...
     $('#btn-download-upload-sample').click(function (e) {
         ...
         $.ajax({
             url: formApiUrl('employee/dmt/contract_job/downloadSample'),
             type: 'get',
             dataType: 'json',
             headers: {
                 Authorization: `Bearer ${wapLogin.getToken()}`,
             ...
             if (res.status == 'success') {
                 if (res.content) {
                     let fileName = 'contractjob-' + moment().format('DD/MM/YYYY') + '.xlsx';

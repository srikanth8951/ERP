...
$(function () {
    window.wapLogin = {
        ...
        setData: function (data) {
            return new Promise(function (resolve, reject) {
                let baseUrl; let rememberMe; let authToken;

                if (parseValue(data.check) == 1) {
                    baseUrl = $.cookie('base_url', base_url, { expires: 7, path: domain_name });
                    rememberMe = $.cookie('remember_me', data.check, { expires: 7, path: domain_name });
                    authToken = $.cookie('auth_token', data.auth_token, { expires: 7, path: domain_name });
                } else {
                    baseUrl = window.sessionStorage.setItem('base_url', base_url);
                    rememberMe = window.sessionStorage.setItem('remember_me', data.check);
                    authToken = window.sessionStorage.setItem('auth_token', data.auth_token);
                }
                
                Promise.all([baseUrl, rememberMe, authToken]).then(function () {
                    resolve(true);
                }).catch(function () {
                    reject(true);
                });
        ...
        check: function () {
            ...
            $.ajax({
                url: formApiUrl('employee/checkLoggedin'),
                type: 'post',
                headers: {
        ...
        showPrompt: function (dfdPromise) {
            ...
            var loginModalContent = `<div class="card border-0">
                <div class="card-body">
                    <h3 class="text-center m-0">
                        <a href="javascript:void(0)" class="logo logo-employee img-fluid"><img class="img-fluid" src="${formUrl('assets/images/sterling-wilson.png')}" alt="logo" /></a>
                    </h3>
            
                    <div class="d-block">
                        <p class="text-muted text-center">Sign in to continue.</p>
                        <form class="form-horizontal m-t-30" id="empLoginForm" method="post">
                            <div class="form-group text-left">
                                <input type="email" name="user_name" class="form-control" id="user_name" placeholder="Usename/Email/Mobile"
                                    required />
                            </div>
                            <div class="form-group text-left">
                            ...
                            <div class="form-group m-t-10 mb-0 row">
                                <div class="col-12 m-t-20">
                                    <a href="${formUrl('recover_password')}" class="text-muted"><i class="mdi mdi-lock"></i>
            ...
            loginForm.submit(function (e) {
                ...
                if (valid) {
                    ...
                    $.when(wapLogin.login(loginForm)).done(function (res) {
        ...
        logout: function () {
            ...
            $.ajax({
                url: formApiUrl('employee/logout'),
                type: 'post',
                dataType: 'json',
                headers: {
                    Authorization: `Bearer ${this.getToken()}`
    ...
    // Logout
    $(document).find(appLogout).click(function (e) {
        e.preventDefault();
        wapLogin.logout()
            .then(function (res) {
                if (res.status == 'success') {

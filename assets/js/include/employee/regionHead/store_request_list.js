...
$(function () {
  ...
  const listContainer = tableContainer.find(
    '[data-container="requestTlistArea"]'
  );
  const listPagination = tableContainer.find(
    '[data-pagination="requestTlistArea"]'
  );
            ...
            if (details.length && pagination.total > 0) {
              ...
              $.each(details, function (listIn, listVal) {
                if (
                  ...
                  if (
                    ((listVal.status.id == 3 && listVal.status.id != 4) ||
                      (listVal.status.id == 5 && listVal.status.id != 6)) &&
                    listVal.status.id != 7 &&
                    listVal.status.id != 8
                  ) {
                    actionBtnApprove =
                      '<button class="text-white btn btn-sm btn-success waves-effect waves-light mr-1" title="Approve" id="btn-request-approval" data-request-id="' +
                      listVal.request_id +
                      '"><i class="mdi mdi-check"></i></button>';
                    actionBtnReject =
                      '<button class="text-white btn btn-sm btn-danger waves-effect waves-light mr-1" title="Reject" id="btn-request-reject" data-request-id="' +
                      listVal.request_id +
                ...
                let url = formUrl(
                  "employee/regionHead/store_request/edit/" + listVal.request_id
                );
                listContainer.append(
                  "<tr>" +
                    "<td>" +
                    listVal.request_number +
                    "</td>" +
                    "<td>" +
                    listVal.title +
                    "</td>" +
                    ...
                    "</td>" +
                    "<td>" +
                    listVal.enginner_name +
                    "</td>" +
                    "<td>" +
                    listVal.created_datetime +
                    "</td>" +
                    "<td>" +
                    '<a href="' +
                    formUrl(
                      "employee/regionHead/store_request/view/" +
                        listVal.request_id
                    ) +
                    '" class="text-white btn btn-sm btn-info waves-effect waves-light mr-1" data-toggle="tooltip" data-placement="top" title=""><i class="mdi mdi-eye"></i></a>' +
                    actionBtnApprove +
                    actionBtnReject +
                    "</td>" +
                    "</tr>"
              ...
              listPagination.find(".list-pagination").pagination({
                ...
                onPageClick: function (pageNumber, event) {
                  var page_link = formApiUrl(
                    "employee/regionalHead/store_product/request/list",
                    {
  ...
  lengthContainer.change(function () {
    loadDetails(formApiUrl("employee/regionalHead/store_product/request/list")); // Load request details
  });
  ...
  $(listContainer).on("click", "#btn-request-approval", function (e) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        $.ajax({
          url: formApiUrl(
            "employee/regionalHead/store_product/request/approval",
            {
          ...
          .done(function (res) {
            if (res.status == "success") {
              loadDetails(
                formApiUrl("employee/regionalHead/store_product/request/list")
  ...
  $(listContainer).on("click", "#btn-request-reject", function (e) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        $.ajax({
          url: formApiUrl(
            "employee/regionalHead/store_product/request/reject",
            {
              request_id: request_id,
            }
          ),

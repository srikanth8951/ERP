...
$(function () {
    ...
    function loadAssetChecklistView(element, data) {
        ...
        element.find('.asset-checklist-body .checklists-area').prepend(`<li class="checklist-row d-flex align-items-center justify-content-between border rounded p-2 mt-2">
            <input type="hidden" name="assets[${data.assetCount}][checklist][${data.checklistCode}][]" value="${data.id}" />
            <span>${data.name}</span>
            <button type="button" data-asset-count="${data.assetCount}" data-checklist-code="${data.checklistCode}" class="btn-remove-checklist btn btn-sm btn-outline-danger waves-effect" ${buttonAttribute}><i class="mdi mdi-delete"></i></button>
        </li>`);
    }
    ...
    // Render checklist into asset view
    window.renderAssetChecklist = async function (assetCount, checklist) {
        let checklistContent = `<div class="col-12 col-md-6" id="assets${assetCount}-checklist${checklist.code}-block">
            <div class="card">
                <div class="card-header shadow-sm border-0">
                    <h6 class="m-0"><a class="d-block btn-collapse-with-icon" data-toggle="collapse" href="#assets${assetCount}-checklist${checklist.code}-collapsez">${checklist.name}
                        <div class="pull-right"><i data-collapse-icon class="mdi mdi-chevron-right"></i></div>
                    </a></h6>
                </div>
                <div class="collapse" id="assets${assetCount}-checklist${checklist.code}-collapsez">
                    <div class="card-body asset-checklist-body">
                        <div class="card position-relative mb-2">
                            <div style="min-height: 50px;" class="card-header p-2 d-flex align-items-center justify-content-between">
                                <p class="m-0">List</p>
                                <button type="button" data-toggle="collapse" data-target="#assets${assetCount}-checklist${checklist.code}" class="starter btn btn-sm btn-link waves-effect waves-light"><i class="mdi mdi-plus"></i>&nbsp;Add</button>
                            </div>
                            <div style="position: absolute;top: 41px;left: 0;right: 0;z-index: 9;" class="collapse collapse-checklist-area" id="assets${assetCount}-checklist${checklist.code}">
                                <div class="card-body bg-light shadow-sm bordered rounded">    
                                    <div class="d-flex align-items-center w-100">
                                        <select data-toggle="jy-select2-assign-checklist${checklist.code}" data-asset-count="${assetCount}" data-checklist-code="${checklist.code}" class="select2"></select>
                                        <span data-toggle="collapse" data-target="#assets${assetCount}-checklist${checklist.code}" class="finisher ml-2 font-20 text-danger"><i class="mdi mdi-close-circle"></i></span>
                                    </div>
                                </div>
                            </div>
                        </div>
        ...
        $('#asset-row' + assetCount + ' .checklists--area').append(checklistContent);

        //  Collapse Icon
        $('#assets' + assetCount + '-checklist'+ checklist.code +'-collapsez').on('show.bs.collapse', function (e) {
        ...
        $('#assets' + assetCount + '-checklist'+ checklist.code +'-collapsez').on('hide.bs.collapse', function (e) {
            let collapseId = $(e.currentTarget).attr('id');
        ...
        //  Collapse Btn
        $('#assets' + assetCount + '-checklist' + checklist.code).on('show.bs.collapse', function (e) {
            let collapseId = $(e.currentTarget).attr('id');
            
            $('[data-target="#'+ collapseId +'"].starter').hide();
        });

        $('#assets' + assetCount + '-checklist' + checklist.code).on('hide.bs.collapse', function (e) {
            let collapseId = $(e.currentTarget).attr('id');
        ...
        // select2
        $('#asset-row' + assetCount + ' [data-toggle="jy-select2-assign-checklist' + checklist.code + '"]').select2({
            // minimumInputLength:2,
            placeholder: "Select a checklist",
            ajax: {
                url: formApiUrl('admin/checklist/autocomplete', { limit: 10, group: checklist.code }),
                dataType: 'json',
                type: 'get',
                delay: 250,
                headers: {
        ...
        $('#asset-row' + assetCount + ' [data-toggle="jy-select2-assign-checklist'+ checklist.code +'"]').on('select2:select', function (e) {
            var data = e.params.data;
            let assetCount = $(e.currentTarget).attr('data-asset-count');
            let checklistCode = $(e.currentTarget).attr('data-checklist-code');
            loadAssetChecklistView($('#assets'+ assetCount +'-checklist'+ checklist.code +'-block'), {
                assetCount: assetCount,
                id: data.id,
                name: data.text,
            ...
            $('[data-target="#assets' + assetCount + '-checklist' + checklistCode + '"].finisher').trigger('click');
    ...
    window.loadAssetView = async function (options) {
                ...
                <div class="card m-b-20" style="border-top: 4px solid rgba(112, 112, 112, 0.12);">
                    <div class="card-body">
                        ...
                        <div class="row">
                            ...
                            <div class="col-12"><div class="checklists--area row"></div></div>
                        </div>
                    </div>
        ...
        await renderAssetChecklist(assetCount, {
            code: 'ppm',
            name: 'PPM Checklist'
        });

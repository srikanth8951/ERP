...
$(function () {
    ...
    window.loadContractJobDetails = function (href) {
        ...
        // Set params in url
        Url.searchParams.set('contract_type_id', contract_type_id);
                    ...
                    if (res.contract_jobs) {
                        ...
                        if (details.length && pagination.total > 0) {
                            ...
                            $.each(details, function (listIn, listVal) {
                                ...
                                var contractJobViewLink = formUrl('admin/contract_job/view/' + listVal.contract_job_id);
                                ...
                                if(parseValue(listVal.is_parent) == '') {
                                    if(expiry_days < 1) {
                                        contractJobRenewLink = formUrl('admin/contract_job/renew/' + listVal.contract_job_id);
                                        ...
                                        contractJobUpdateLink = formUrl('admin/contract_job/update/' + listVal.contract_job_id);
                            ...
                            listPagination.find(".list-pagination").pagination({
                                ...
                                onPageClick: function (pageNumber, event) {
                                    var page_link = formApiUrl('admin/contract_job/list', { start: ((parseInt(pagination.length) * (pageNumber - 1)) + 1)});
    ...
    searchForm.submit(function (e) {
        e.preventDefault();
        loadContractJobDetails(formApiUrl('admin/contract_job/list'));  // Load contract job details
    });
    ...
    $(listContainer).on('click', '.btn-switch-contractjob-status', function (e) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    url: formApiUrl('admin/contract_job/status/update', { contract_job_id: contractjob_id }),
                    type: 'post',
                    dataType: 'json',
                    data: { status: status },
                    headers: {
                    ...
                    if (res.status == 'success') {
                        loadContractJobDetails(formApiUrl('admin/contract_job/list'));  // Load employee details
    ...
    $('#btn-download-upload-sample').click(function (e) {
        ...
        $.ajax({
            url: formApiUrl('admin/contract_job/downloadSample'),
            type: 'get',
            dataType: 'json',
            headers: {
                Authorization: `Bearer ${wapLogin.getToken()}`,

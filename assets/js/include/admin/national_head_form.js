...
$(function () {
  ...
  const employeeNationalHeadForm = $("#employeeNationalHeadForm");
  var employeeNationalHeadFormValidator;
  ...
  window.loadAutocompleteDesignations = function (selected = []) {
    let moduleDesignation = new ModuleDesignation({
      ...
      selectboxElement: employeeNationalHeadForm.find(
        '[name="designation_id"]'
      ),
    });
  ...
  window.loadAutocompleteDepartments = function (selected = []) {
    let moduleDepartment = new ModuleDepartment({
      ...
      selectboxElement: employeeNationalHeadForm.find('[name="department_id"]'),
    });
  ...
  employeeNationalHeadForm.on(
    "click",
    "#btn-add-user-designation",
    function (e) {
      e.preventDefault();
      let moduleDesignation = new ModuleDesignation({
        autoloadUrl: formApiUrl("admin/designation/autocomplete"),
      ...
      moduleDesignation.loadPrompt({
        selected: [
          employeeNationalHeadForm.find('[name="designation_id"]').val(),
        ],
  ...
  employeeNationalHeadForm.on(
    ...
    function (e) {
      ...
      moduleDepartment.loadPrompt({
        selected: [
          employeeNationalHeadForm.find('[name="department_id"]').val(),
        ],
  ...
  employeeNationalHeadForm.on(
    ...
    function (e) {
      ...
      moduleWorkExpertise.loadPrompt({
        selected: [
          employeeNationalHeadForm.find('[name="work_expertise"]').val(),
        ],
  ...
  window.loadAutocompleteCountry = function (options = {}) {
    var selected;
    var countrySelectbox = employeeNationalHeadForm.find('[name="country_id"]');
  ...
  window.loadAutocompleteStates = function (options = {}) {
    var selected;
    var country_id;
    var stateSelectbox = employeeNationalHeadForm.find('[name="state_id"]');
  ...
  window.loadAutocompleteCities = function (options = {}) {
    var selected;
    var state_id;
    var citySelectbox = employeeNationalHeadForm.find('[name="city_id"]');
  ...
  if (emp_form_type == "edit") {
    ...
    employeeNationalHeadForm.find('[name="country_id"]').change(function () {
      if (parseValue($(this).val()) != "") {
  ...
  if (emp_form_type == "edit") {
    ...
    // passing state ID to city
    employeeNationalHeadForm.find('[name="state_id"]').change(function () {
      if (parseValue($(this).val()) != "") {
        loadAutocompleteCities({
          state_id: $(this).val(),
        });
      } else {
        employeeNationalHeadForm.find('[name="city_id"]').html("").trigger("change");
      }
    });
  }

  employeeNationalHeadForm.find(".password-generate").click(function (e) {
    e.preventDefault();
    let res = generateRandomString(8);
    employeeNationalHeadForm.find('[name="password"]').val(res);
  });
  ...
  window.loadEmpDetail = function (href) {
    ...
    $.ajax({
      url: formApiUrl("admin/employee/national_head/detail", {
  ...
  // employee Form
  employeeNationalHeadFormValidator = employeeNationalHeadForm.validate({
  ...
  if (emp_form_type == "edit") {
    employeeNationalHeadForm.find('[name="password"]').rules("remove");
  } else {
    employeeNationalHeadForm.find('[name="password"]').rules("add", {
      required: true,
      messages: {
  ...
  function saveEmployee() {
    let loadSwal;
    var formData = new FormData(employeeNationalHeadForm[0]);
    ...
    $.ajax({
      ...
      .done(function (res) {
        if (res.status == "success") {
          ...
          employeeNationalHeadForm[0].reset(); // Reset form
          setTimeout(function () {
            window.location.href = formUrl("admin/national_head");
          });
        } else if (res.status == "error") {
          toastr.error(res.message);
        } else {
  ...
  employeeNationalHeadForm.submit(function (e) {
    e.preventDefault();
    if (employeeNationalHeadFormValidator.valid()) {
      saveEmployee();
    }
  });
});

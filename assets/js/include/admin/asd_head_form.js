...
$(function () {
  ...
  const employeeASDHeadForm = $("#employeeASDHeadForm");
  var employeeASDHeadFormValidator;
  ...
  window.loadAutocompleteRegions = function (selected = []) {
    let moduleRegion = new ModuleRegion({
      ...
      selectboxElement: employeeASDHeadForm.find('[name="region_id"]'),
    });
  ...
  window.loadAutocompleteBranches = function (selected = []) {
    ...
    let moduleBranch = new ModuleBranch({
      ...
      selectboxElement: employeeASDHeadForm.find('[name="branch_id"]'),
    });
  ...
  window.loadAutocompleteDesignations = function (selected = []) {
    let moduleDesignation = new ModuleDesignation({
      ...
      selectboxElement: employeeASDHeadForm.find('[name="designation_id"]'),
    });
  ...
  window.loadAutocompleteDepartments = function (selected = []) {
    let moduleDepartment = new ModuleDepartment({
      ...
      selectboxElement: employeeASDHeadForm.find('[name="department_id"]'),
    });
  ...
  window.loadAutocompleteWorkExpertise = function (selected = []) {
    let moduleWorkExpertise = new ModuleWorkExpertise({
        ...
        selectboxElement: employeeASDHeadForm.find('[name="work_expertise"]'),
    });
  ...
  // Add user region
  employeeASDHeadForm.on("click", "#btn-add-user-region", function (e) {
    e.preventDefault();
    let moduleRegion = new ModuleRegion({
      autoloadUrl: formApiUrl("admin/region/autocomplete"),
      selectboxElement: employeeASDHeadForm.find('[name="region_id"]'),
    ...
    moduleRegion.loadPrompt({
      selected: [employeeASDHeadForm.find('[name="region_id"]').val()],
      submitAction: formApiUrl("admin/region/add"),
    });
  });

  // Add user branch
  employeeASDHeadForm.on("click", "#btn-add-user-branch", function (e) {
    e.preventDefault();
    let moduleBranch = new ModuleBranch({
      autoloadUrl: formApiUrl("admin/branch/autocomplete"),
      selectboxElement: employeeASDHeadForm.find('[name="branch_id"]'),
    ...
    moduleBranch.loadPrompt({
      selected: [employeeASDHeadForm.find('[name="branch_id"]').val()],
      submitAction: formApiUrl("admin/branch/add"),
    });

    loadAutocompleteRegions();
    employeeASDHeadForm.find('[name="branch_id"]').html("").trigger("change");
  });

  // Add user designation
  employeeASDHeadForm.on("click", "#btn-add-user-designation", function (e) {
    e.preventDefault();
    let moduleDesignation = new ModuleDesignation({
      autoloadUrl: formApiUrl("admin/designation/autocomplete"),
      selectboxElement: employeeASDHeadForm.find('[name="designation_id"]'),
    ...
    moduleDesignation.loadPrompt({
      selected: [employeeASDHeadForm.find('[name="designation_id"]').val()],
      submitAction: formApiUrl("admin/designation/add"),
    });
  });

  // Add user department
  employeeASDHeadForm.on("click", "#btn-add-user-department", function (e) {
    e.preventDefault();
    let moduleDepartment = new ModuleDepartment({
      autoloadUrl: formApiUrl("admin/department/autocomplete"),
      selectboxElement: employeeASDHeadForm.find('[name="department_id"]'),
    ...
    moduleDepartment.loadPrompt({
      selected: [employeeASDHeadForm.find('[name="department_id"]').val()],
      submitAction: formApiUrl("admin/department/add"),
    });
  });

  // Add user WorkExpertise
  employeeASDHeadForm.on('click', '#btn-add-user-work-expertise', function (e) {
    e.preventDefault();
    let moduleWorkExpertise = new ModuleWorkExpertise({
        autoloadUrl: formApiUrl('admin/work_expertise/autocomplete'),
        selectboxElement: employeeASDHeadForm.find('[name="work_expertise"]')
    ...
    moduleWorkExpertise.loadPrompt({
        selected: [employeeASDHeadForm.find('[name="work_expertise"]').val()],
        submitAction: formApiUrl('admin/work_expertise/add')
    });
  });

  // passing region ID to branch
  employeeASDHeadForm.find('[name="region_id"]').on("select2:select", function () {
  ...
  window.loadAutocompleteCountry = function (options = {}) {
    var selected;
    var countrySelectbox = employeeASDHeadForm.find('[name="country_id"]');
  ...
  window.loadAutocompleteStates = function (options = {}) {
    var selected;
    var country_id;
    var stateSelectbox = employeeASDHeadForm.find('[name="state_id"]');
  ...
  window.loadAutocompleteCities = function (options = {}) {
    var selected;
    var state_id;
    var citySelectbox = employeeASDHeadForm.find('[name="city_id"]');
  ...
  if (emp_form_type == "edit") {
    // passing country ID to state
    employeeASDHeadForm.find('[name="country_id"]').on("select2:select", function () {
      if (parseValue($(this).val()) != "") {
        loadAutocompleteStates({
          country_id: $(this).val(),
        });
      } else {
        employeeASDHeadForm.find('[name="state_id"]').html("").trigger("change");
      }
    });

    employeeASDHeadForm.find('[name="country_id"]').change(function () {
      if (parseValue($(this).val()) != "") {
  ...
  if (emp_form_type == "edit") {
    ...
    // passing state ID to city
    employeeASDHeadForm.find('[name="state_id"]').change(function () {
      if (parseValue($(this).val()) != "") {
        loadAutocompleteCities({
          state_id: $(this).val(),
        });
  ...
  employeeASDHeadForm.find(".password-generate").click(function (e) {
    e.preventDefault();
    let res = generateRandomString(8);
    employeeASDHeadForm.find('[name="password"]').val(res);
  });
  ...
  window.loadEmpDetail = function (href) {
    ...
    $.ajax({
      url: formApiUrl("admin/employee/asd_head/detail", {
      ...
      success: function (res) {
        if (res.status == "success") {
          ...
          if (parseValue(res.employee.data) != "") {
            ...
            employeeASDHeadForm.find('[name="email"]').val(empDetail.email);
            employeeASDHeadForm.find('[name="mobile"]').val(empDetail.mobile);
            employeeASDHeadForm.find('[name="address"]').val(empDetail.address);
            employeeASDHeadForm
              .find('[name="username"]')
              .val(empDetail.username);
            employeeASDHeadForm.find('[name="city"]').val(empDetail.city);
            employeeASDHeadForm.find('[name="pincode"]').val(empDetail.pincode);
            employeeASDHeadForm
  ...
  // employee Form
  employeeASDHeadFormValidator = employeeASDHeadForm.validate({
  ...
  if (emp_form_type == "edit") {
    employeeASDHeadForm.find('[name="password"]').rules("remove");
  } else {
    employeeASDHeadForm.find('[name="password"]').rules("add", {
      required: true,
      messages: {
  ...
  function saveEmployee() {
    let loadSwal;
    var formData = new FormData(employeeASDHeadForm[0]);
    ...
    $.ajax({
      ...
      .done(function (res) {
        if (res.status == "success") {
          ...
          employeeASDHeadForm[0].reset(); // Reset form
          setTimeout(function () {
            window.location.href = formUrl("admin/asd_head");
          });
        } else if (res.status == "error") {
          toastr.error(res.message);
        } else {
  ...
  employeeASDHeadForm.submit(function (e) {
    e.preventDefault();
    if (employeeASDHeadFormValidator.valid()) {
      saveEmployee();
    }
  });
});

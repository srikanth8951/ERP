 ...
 */
 ...
 $(function () {
     ...
     const listContainer = listArea.find('[data-container="empStoreArea"]');
     const listPagination = listArea.find('[data-pagination="empStoreArea"]');
                     ...
                     if (res.employees) {
                         ...
                         if (details.length && pagination.total > 0) {
                             ...
                             $.each(details, function (listIn, listVal) {
                                 ...
                                 var employeeViewLink = formUrl('admin/store/users/view/' + listVal.employee_id);
                                 var employeeEditLink = formUrl('admin/store/users/edit/' + listVal.employee_id);
                                 listContainer.append(`<div class="col-md-12">
                                     <div class="card m-b-20 card-body" style="padding: 0.75rem;">
                                         <div class="row align-items-center ">
                                             ...
                                             <div class="float-right">
                                                 ...
                                                 <a href="javascript:void(0);" class="btn-delete-employee-store-user" data-employee="${listVal.employee_id}"><i class="fa fa-trash text-danger"></i></a>
                                             </div>
                                             </div>
                                         </div>
                                     </div>
                            ...
                            $('.clickable').click(function() { 
                             ...
                             listPagination.find(".list-pagination").pagination({
                                 ...
                                 onPageClick: function (pageNumber, event) {
                                     var page_link = formApiUrl('admin/store/user/list', { start: ((parseInt(pagination.length) * (pageNumber - 1)) + 1)});
     ...
     searchForm.submit(function (e) {
         e.preventDefault();
         loadEmpDetails(formApiUrl('admin/store/user/list'));  // Load employee details
     });
     ...
     $(listContainer).on('click', '.btn-switch', function (e) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    url: formApiUrl('admin/store/user/status/update', { employee_id: employee_id }),
                    type: 'post',
                    dataType: 'json',
                    data: {status: status},
                    headers: {
     ...
     // Delete employee
     $(listContainer).on('click', '.btn-delete-employee-store-user', function (e) {
         e.preventDefault();
         var employee_id = $(this).attr('data-employee');
         Swal.fire({
             icon: 'question',
             ...
             if (result.isConfirmed) {
                 ...
                 $.ajax({
                     url: formApiUrl('admin/store/user/delete', { employee_id: employee_id }),
                     type: 'post',
                     dataType: 'json',
                     headers: {
                         Authorization: `Bearer ${wapLogin.getToken()}`

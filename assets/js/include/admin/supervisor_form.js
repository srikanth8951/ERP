...
$(function () {
  ...
  const employeeSupervisorForm = $("#employeeSupervisorForm");
  var employeeSupervisorFormValidator;
  ...
  window.loadAutocompleteRegions = function (selected = []) {
    let moduleRegion = new ModuleRegion({
      ...
      selectboxElement: employeeSupervisorForm.find('[name="region_id"]'),
    });
  ...
  window.loadAutocompleteBranches = function (selected = []) {
    let moduleBranch = new ModuleBranch({
      ...
      selectboxElement: employeeSupervisorForm.find('[name="branch_id"]'),
    });
  ...
  window.loadAutocompleteAreas = function (selected = []) {
    let moduleArea = new ModuleArea({
      ...
      selectboxElement: employeeSupervisorForm.find('[name="area_id"]'),
    });
  ...
  window.loadAutocompleteDesignations = function (selected = []) {
    let moduleDesignation = new ModuleDesignation({
      ...
      selectboxElement: employeeSupervisorForm.find('[name="designation_id"]'),
    });
  ...
  window.loadAutocompleteDepartments = function (selected = []) {
    let moduleDepartment = new ModuleDepartment({
      ...
      selectboxElement: employeeSupervisorForm.find('[name="department_id"]'),
    });
  ...
  window.loadAutocompleteWorkExpertise = function (selected = []) {
    let moduleWorkExpertise = new ModuleWorkExpertise({
      ...
      selectboxElement: employeeSupervisorForm.find('[name="work_expertise"]'),
    });
  ...
  // Add user region
  employeeSupervisorForm.on("click", "#btn-add-user-region", function (e) {
    e.preventDefault();
    let moduleRegion = new ModuleRegion({
      autoloadUrl: formApiUrl("admin/region/autocomplete"),
      selectboxElement: employeeSupervisorForm.find('[name="region_id"]'),
    ...
    moduleRegion.loadPrompt({
      selected: [employeeSupervisorForm.find('[name="region_id"]').val()],
      submitAction: formApiUrl("admin/region/add"),
    });
  });

  // Add user branch
  employeeSupervisorForm.on("click", "#btn-add-user-branch", function (e) {
    e.preventDefault();
    let moduleBranch = new ModuleBranch({
      autoloadUrl: formApiUrl("admin/branch/autocomplete"),
      selectboxElement: employeeSupervisorForm.find('[name="branch_id"]'),
    ...
    moduleBranch.loadPrompt({
      selected: [employeeSupervisorForm.find('[name="branch_id"]').val()],
    ...
    employeeSupervisorForm
      ...
      .html("")
      .trigger("change");
    employeeSupervisorForm.find('[name="area_id"]').html("").trigger("change");
  });

  // Add user designation
  employeeSupervisorForm.on("click", "#btn-add-user-designation", function (e) {
    e.preventDefault();
    let moduleDesignation = new ModuleDesignation({
      autoloadUrl: formApiUrl("admin/designation/autocomplete"),
      selectboxElement: employeeSupervisorForm.find('[name="designation_id"]'),
    ...
    moduleDesignation.loadPrompt({
      selected: [employeeSupervisorForm.find('[name="designation_id"]').val()],
      submitAction: formApiUrl("admin/designation/add"),
    });
  });

  // Add user department
  employeeSupervisorForm.on("click", "#btn-add-user-department", function (e) {
    e.preventDefault();
    let moduleDepartment = new ModuleDepartment({
      autoloadUrl: formApiUrl("admin/department/autocomplete"),
      selectboxElement: employeeSupervisorForm.find('[name="department_id"]'),
    ...
    moduleDepartment.loadPrompt({
      selected: [employeeSupervisorForm.find('[name="department_id"]').val()],
      submitAction: formApiUrl("admin/department/add"),
    });
  });

  // Add user WorkExpertise
  employeeSupervisorForm.on(
    "click",
    ...
    function (e) {
      ...
      let moduleWorkExpertise = new ModuleWorkExpertise({
        autoloadUrl: formApiUrl("admin/work_expertise/autocomplete"),
        selectboxElement: employeeSupervisorForm.find(
          '[name="work_expertise"]'
        ),
      });
      ...
      moduleWorkExpertise.loadPrompt({
        selected: [
          employeeSupervisorForm.find('[name="work_expertise"]').val(),
        ],
  ...
  // Add user area
  employeeSupervisorForm.on("click", "#btn-add-user-area", function (e) {
    e.preventDefault();
    let moduleArea = new ModuleArea({
      autoloadUrl: formApiUrl("admin/area/autocomplete"),
      selectboxElement: employeeSupervisorForm.find('[name="area_id"]'),
    ...
    moduleArea.loadPrompt({
      selected: [employeeSupervisorForm.find('[name="area_id"]').val()],
    ...
    .on("select2:select", function () {
      if (parseValue($(this).val()) != "") {
        loadAutocompleteAddressStates({
          element: employeeSupervisorForm.find('[name="state_id"]'),
          params: {
            country_id: $(this).val(),
          },
        });
  ...
  employeeSupervisorForm.find('[name="country_id"]').change(function () {
    if (parseValue($(this).val()) != "") {
    ...
    .on("select2:select", function () {
      if (parseValue($(this).val()) != "") {
        loadAutocompleteAddressCities({
          element: employeeSupervisorForm.find('[name="city_id"]'),
          params: {
            state_id: $(this).val(),
          },
        });
  ...
  employeeSupervisorForm.find(".password-generate").click(function (e) {
    e.preventDefault();
    let res = generateRandomString(8);
    employeeSupervisorForm.find('[name="password"]').val(res);
  });
  ...
  window.loadEmpDetail = function (href) {
    ...
    $.ajax({
      url: formApiUrl("admin/employee/supervisor/detail", {
      ...
      success: async function (res) {
        if (res.status == "success") {
          ...
          if (parseValue(res.employee.data) != "") {
            ...
            employeeSupervisorForm.find('[name="email"]').val(empDetail.email);
            employeeSupervisorForm
              .find('[name="mobile"]')
            ...
            loadAutocompleteAddressCountries({
              element: employeeSupervisorForm.find('[name="country_id"]'),
              params: {
  ...
  // employee Form
  employeeSupervisorFormValidator = employeeSupervisorForm.validate({
  ...
  if (emp_form_type == "edit") {
    employeeSupervisorForm.find('[name="password"]').rules("remove");
  } else {
    employeeSupervisorForm.find('[name="password"]').rules("add", {
      required: true,
      messages: {
  ...
  function saveEmployee() {
    let loadSwal;
    var formData = new FormData(employeeSupervisorForm[0]);
    ...
    $.ajax({
      ...
      .done(function (res) {
        if (res.status == "success") {
          ...
          employeeSupervisorForm[0].reset(); // Reset form
          setTimeout(function () {
            window.location.href = formUrl("admin/supervisor");
          });
        } else if (res.status == "error") {
          toastr.error(res.message);
        } else {
  ...
  employeeSupervisorForm.submit(function (e) {
    e.preventDefault();
    if (employeeSupervisorFormValidator.valid()) {
      saveEmployee();
    }
  });
});

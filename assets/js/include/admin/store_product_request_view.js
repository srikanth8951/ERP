...
$(function () {
    ...
    if (parseValue(detail) != "" && Object.keys(detail).length > 0) {
      ...
      if (detail.products) {
        ...
        $.each(detail.products, function (listIn, listVal) {
          ...
          let stock_status = ''
          let stock_tip = ''
          if (listVal.quantity >= listVal.requested_quantity) {
          ...
          <tbody>
            <tr>
                ...
                <td>${listVal.requested_quantity} / <span class="${stock_status}" title="${stock_tip}"> ${ listVal.quantity }</span></td>
                <td>${total_price}</td>
            </tr>
          </tbody>`);
        });
  ...
  window.loadRequestDetail = function () {
    ...
    $.ajax({
      url: formApiUrl("admin/store/product/request/detail", {

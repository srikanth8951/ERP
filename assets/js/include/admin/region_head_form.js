...
$(function () {
  ...
  const employeeRegionHeadForm = $("#employeeRegionHeadForm");
  var employeeRegionHeadFormValidator;
  ...
  window.loadAutocompleteRegions = function (selected = []) {
    let moduleRegion = new ModuleRegion({
      ...
      selectboxElement: employeeRegionHeadForm.find('[name="region_id"]'),
    });
  ...
  window.loadAutocompleteDesignations = function (selected = []) {
    let moduleDesignation = new ModuleDesignation({
      ...
      selectboxElement: employeeRegionHeadForm.find('[name="designation_id"]'),
    });
  ...
  window.loadAutocompleteDepartments = function (selected = []) {
    let moduleDepartment = new ModuleDepartment({
      ...
      selectboxElement: employeeRegionHeadForm.find('[name="department_id"]'),
    });
  ...
  window.loadAutocompleteWorkExpertise = function (selected = []) {
    let moduleWorkExpertise = new ModuleWorkExpertise({
      ...
      selectboxElement: employeeRegionHeadForm.find('[name="work_expertise"]'),
    });
  ...
  // Add user region
  employeeRegionHeadForm.on("click", "#btn-add-user-region", function (e) {
    e.preventDefault();
    let moduleRegion = new ModuleRegion({
      autoloadUrl: formApiUrl("admin/region/autocomplete"),
      selectboxElement: employeeRegionHeadForm.find('[name="region_id"]'),
    ...
    moduleRegion.loadPrompt({
      selected: [employeeRegionHeadForm.find('[name="region_id"]').val()],
      submitAction: formApiUrl("admin/region/add"),
    });
  });

  // Add user designation
  employeeRegionHeadForm.on("click", "#btn-add-user-designation", function (e) {
    e.preventDefault();
    let moduleDesignation = new ModuleDesignation({
      autoloadUrl: formApiUrl("admin/designation/autocomplete"),
      selectboxElement: employeeRegionHeadForm.find('[name="designation_id"]'),
    ...
    moduleDesignation.loadPrompt({
      selected: [employeeRegionHeadForm.find('[name="designation_id"]').val()],
      submitAction: formApiUrl("admin/designation/add"),
    });
  });

  // Add user department
  employeeRegionHeadForm.on("click", "#btn-add-user-department", function (e) {
    e.preventDefault();
    let moduleDepartment = new ModuleDepartment({
      autoloadUrl: formApiUrl("admin/department/autocomplete"),
      selectboxElement: employeeRegionHeadForm.find('[name="department_id"]'),
    ...
    moduleDepartment.loadPrompt({
      selected: [employeeRegionHeadForm.find('[name="department_id"]').val()],
      submitAction: formApiUrl("admin/department/add"),
    });
  });

  // Add user WorkExpertise
  employeeRegionHeadForm.on(
    "click",
    ...
    function (e) {
      ...
      let moduleWorkExpertise = new ModuleWorkExpertise({
        autoloadUrl: formApiUrl("admin/work_expertise/autocomplete"),
        selectboxElement: employeeRegionHeadForm.find(
          '[name="work_expertise"]'
        ),
      });
      ...
      moduleWorkExpertise.loadPrompt({
        selected: [
          employeeRegionHeadForm.find('[name="work_expertise"]').val(),
        ],
  ...
  window.loadAutocompleteCountry = function (options = {}) {
    var selected;
    var countrySelectbox = employeeRegionHeadForm.find('[name="country_id"]');
  ...
  window.loadAutocompleteStates = function (options = {}) {
    var selected;
    var country_id;
    var stateSelectbox = employeeRegionHeadForm.find('[name="state_id"]');
  ...
  window.loadAutocompleteCities = function (options = {}) {
    var selected;
    var state_id;
    var citySelectbox = employeeRegionHeadForm.find('[name="city_id"]');
  ...
  if (emp_form_type == "edit") {
    ...
    employeeRegionHeadForm.find('[name="country_id"]').change(function () {
      if (parseValue($(this).val()) != "") {
  ...
  if (emp_form_type == "edit") {
    ...
    // passing state ID to city
    employeeRegionHeadForm.find('[name="state_id"]').change(function () {
      if (parseValue($(this).val()) != "") {
        loadAutocompleteCities({
          state_id: $(this).val(),
        });
      } else {
        employeeRegionHeadForm.find('[name="city_id"]').html("").trigger("change");
      }
    });
  }

  employeeRegionHeadForm.find(".password-generate").click(function (e) {
    e.preventDefault();
    let res = generateRandomString(8);
    employeeRegionHeadForm.find('[name="password"]').val(res);
  });
  ...
  window.loadEmpDetail = function (href) {
    ...
    $.ajax({
      url: formApiUrl("admin/employee/region_head/detail", {
      ...
      success: function (res) {
        if (res.status == "success") {
          ...
          if (parseValue(res.employee.data) != "") {
            ...
            employeeRegionHeadForm.find('[name="email"]').val(empDetail.email);
            employeeRegionHeadForm
              .find('[name="mobile"]')
            ...
            // loadAutocompleteBranches({ region_id: empDetail.region_id, selected: [empDetail.branch_id] });
  ...
  // employee Form
  employeeRegionHeadFormValidator = employeeRegionHeadForm.validate({
  ...
  if (emp_form_type == "edit") {
    employeeRegionHeadForm.find('[name="password"]').rules("remove");
  } else {
    employeeRegionHeadForm.find('[name="password"]').rules("add", {
      required: true,
      messages: {
  ...
  function saveEmployee() {
    let loadSwal;
    var formData = new FormData(employeeRegionHeadForm[0]);
    ...
    $.ajax({
      ...
      .done(function (res) {
        if (res.status == "success") {
          ...
          employeeRegionHeadForm[0].reset(); // Reset form
          setTimeout(function () {
            window.location.href = formUrl("admin/regional_head");
          });
        } else if (res.status == "error") {
          toastr.error(res.message);
        } else {
  ...
  employeeRegionHeadForm.submit(function (e) {
    e.preventDefault();
    if (employeeRegionHeadFormValidator.valid()) {
      saveEmployee();
    }
  });
});

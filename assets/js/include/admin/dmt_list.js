 ...
 */
 ...
 $(function () {
     ...
     const listArea = $('#empdmt--deatils--area');
     const listContainer = listArea.find('[data-container="empDMTArea"]');
     const listPagination = listArea.find('[data-pagination="empDMTArea"]');
                     ...
                     if (res.employees) {
                         ...
                         if (details.length && pagination.total > 0) {
                             ...
                             $.each(details, function (listIn, listVal) {
                                 ...
                                 var employeeViewLink = formUrl('admin/dmt/view/' + listVal.employee_id);
                                 var employeeEditLink = formUrl('admin/dmt/edit/' + listVal.employee_id);
                                 listContainer.append(`<div class="col-md-12">
                                     <div class="card m-b-20 card-body" style="padding: 0.75rem;">
                                         <div class="row align-items-center ">
                                             ...
                                             <div class="float-right">
                                                 ...
                                                 <a href="javascript:void(0);" class="btn-delete-employee-dmt" data-employee="${listVal.employee_id}"><i class="fa fa-trash text-danger"></i></a>
                                             </div>
                                             </div>
                                         </div>
                                     </div>
                            ...
                            $('.clickable').click(function() { 
                             ...
                             listPagination.find(".list-pagination").pagination({
                                 ...
                                 onPageClick: function (pageNumber, event) {
                                     var page_link = formApiUrl('admin/employee/data_management/list', { start: ((parseInt(pagination.length) * (pageNumber - 1)) + 1)});
     ...
     searchForm.submit(function (e) {
         e.preventDefault();
         loadEmpDetails(formApiUrl('admin/employee/data_management/list'));  // Load employee details
     });
     ...
     $(listContainer).on('click', '.btn-switch', function (e) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    url: formApiUrl('admin/employee/data_management/status/update', { employee_id: employee_id }),
                    type: 'post',
                    dataType: 'json',
                    data: {status: status},
                    headers: {
     ...
     // Delete employee
     $(listContainer).on('click', '.btn-delete-employee-dmt', function (e) {
         e.preventDefault();
         var employee_id = $(this).attr('data-employee');
         Swal.fire({
             icon: 'question',
             ...
             if (result.isConfirmed) {
                 ...
                 $.ajax({
                     url: formApiUrl('admin/employee/data_management/delete', { employee_id: employee_id }),
                     type: 'post',
                     dataType: 'json',
                     headers: {
                         Authorization: `Bearer ${wapLogin.getToken()}`
     ...
     $('#btn-download-upload-sample').click(function (e) {
        ...
        $.ajax({
            url: formApiUrl('admin/employee/DataManagement/downloadSample'),
            type: 'get',
            dataType: 'json',
            headers: {
                Authorization: `Bearer ${wapLogin.getToken()}`,
            ...
            if (res.status == 'success') {
                if (res.content) {
                    let fileName = 'dataManagement-' + moment().format('DD/MM/YYYY') + '.xlsx';

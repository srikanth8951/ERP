...
$(function () {
  ...
  const employeeManagerForm = $("#employeeManagerForm");
  var employeeManagerFormValidator;
  ...
  window.loadAutocompleteRegions = function (selected = []) {
    let moduleRegion = new ModuleRegion({
      ...
      selectboxElement: employeeManagerForm.find('[name="region_id"]'),
    });
  ...
  window.loadAutocompleteBranches = function (selected = []) {
    let moduleBranch = new ModuleBranch({
      ...
      selectboxElement: employeeManagerForm.find('[name="branch_id"]'),
    });
  ...
  window.loadAutocompleteAreas = function (selected = []) {
    let moduleArea = new ModuleArea({
      ...
      selectboxElement: employeeManagerForm.find('[name="area_id"]'),
    });
  ...
  window.loadAutocompleteDesignations = function (selected = []) {
    let moduleDesignation = new ModuleDesignation({
      ...
      selectboxElement: employeeManagerForm.find('[name="designation_id"]'),
    });
  ...
  window.loadAutocompleteDepartments = function (selected = []) {
    let moduleDepartment = new ModuleDepartment({
      ...
      selectboxElement: employeeManagerForm.find('[name="department_id"]'),
    });
  ...
  window.loadAutocompleteWorkExpertise = function (selected = []) {
    let moduleWorkExpertise = new ModuleWorkExpertise({
      ...
      selectboxElement: employeeManagerForm.find('[name="work_expertise"]'),
    });
  ...
  // Add user region
  employeeManagerForm.on("click", "#btn-add-user-region", function (e) {
    e.preventDefault();
    let moduleRegion = new ModuleRegion({
      autoloadUrl: formApiUrl("admin/region/autocomplete"),
      selectboxElement: employeeManagerForm.find('[name="region_id"]'),
    ...
    moduleRegion.loadPrompt({
      selected: [employeeManagerForm.find('[name="region_id"]').val()],
      submitAction: formApiUrl("admin/region/add"),
    });
  });

  // Add user branch
  employeeManagerForm.on("click", "#btn-add-user-branch", function (e) {
    e.preventDefault();
    let moduleBranch = new ModuleBranch({
      autoloadUrl: formApiUrl("admin/branch/autocomplete"),
      selectboxElement: employeeManagerForm.find('[name="branch_id"]'),
    ...
    moduleBranch.loadPrompt({
      selected: [employeeManagerForm.find('[name="branch_id"]').val()],
      submitAction: formApiUrl("admin/branch/add"),
    });
    loadAutocompleteRegions();
    employeeManagerForm.find('[name="branch_id"]').html("").trigger("change");
    employeeManagerForm.find('[name="area_id"]').html("").trigger("change");
  });

  // Add user designation
  employeeManagerForm.on("click", "#btn-add-user-designation", function (e) {
    e.preventDefault();
    let moduleDesignation = new ModuleDesignation({
      autoloadUrl: formApiUrl("admin/designation/autocomplete"),
      selectboxElement: employeeManagerForm.find('[name="designation_id"]'),
    ...
    moduleDesignation.loadPrompt({
      selected: [employeeManagerForm.find('[name="designation_id"]').val()],
      submitAction: formApiUrl("admin/designation/add"),
    });
  });

  // Add user department
  employeeManagerForm.on("click", "#btn-add-user-department", function (e) {
    e.preventDefault();
    let moduleDepartment = new ModuleDepartment({
      autoloadUrl: formApiUrl("admin/department/autocomplete"),
      selectboxElement: employeeManagerForm.find('[name="department_id"]'),
    ...
    moduleDepartment.loadPrompt({
      selected: [employeeManagerForm.find('[name="department_id"]').val()],
      submitAction: formApiUrl("admin/department/add"),
    });
  });

  // Add user WorkExpertise
  employeeManagerForm.on(
    "click",
    ...
    function (e) {
      ...
      moduleWorkExpertise.loadPrompt({
        selected: [employeeManagerForm.find('[name="work_expertise"]').val()],
  ...
  // Add user area
  employeeManagerForm.on("click", "#btn-add-user-area", function (e) {
    e.preventDefault();
    let moduleArea = new ModuleArea({
      autoloadUrl: formApiUrl("admin/area/autocomplete"),
      selectboxElement: employeeManagerForm.find('[name="area_id"]'),
    ...
    moduleArea.loadPrompt({
      selected: [employeeManagerForm.find('[name="area_id"]').val()],
    ...
    .on("select2:select", function () {
      if (parseValue($(this).val()) != "") {
        loadAutocompleteAddressStates({
          element: employeeManagerForm.find('[name="state_id"]'),
          params: {
            country_id: $(this).val(),
          },
        });
  ...
  employeeManagerForm.find('[name="country_id"]').change(function () {
    if (parseValue($(this).val()) != "") {
    ...
    .on("select2:select", function () {
      if (parseValue($(this).val()) != "") {
        loadAutocompleteAddressCities({
          element: employeeManagerForm.find('[name="city_id"]'),
          params: {
            state_id: $(this).val(),
          },
        });
  ...
  employeeManagerForm.find(".password-generate").click(function (e) {
    e.preventDefault();
    let res = generateRandomString(8);
    employeeManagerForm.find('[name="password"]').val(res);
  });
  ...
  window.loadEmpDetail = function (href) {
    ...
    $.ajax({
      url: formApiUrl("admin/employee/manager/detail", {
      ...
      success: async function (res) {
        if (res.status == "success") {
          ...
          if (parseValue(res.employee.data) != "") {
            ...
            employeeManagerForm.find('[name="email"]').val(empDetail.email);
            employeeManagerForm.find('[name="mobile"]').val(empDetail.mobile);
            employeeManagerForm
              .find('[name="username"]')
              .val(empDetail.username);
            employeeManagerForm.find('[name="address"]').val(empDetail.address);
            employeeManagerForm.find('[name="pincode"]').val(empDetail.pincode);
            employeeManagerForm
            ...
            loadAutocompleteAddressCountries({
              element: employeeManagerForm.find('[name="country_id"]'),
              params: {
  ...
  // employee Form
  employeeManagerFormValidator = employeeManagerForm.validate({
  ...
  if (emp_form_type == "edit") {
    employeeManagerForm.find('[name="password"]').rules("remove");
  } else {
    employeeManagerForm.find('[name="password"]').rules("add", {
      required: true,
      messages: {
  ...
  function saveEmployee() {
    let loadSwal;
    var formData = new FormData(employeeManagerForm[0]);
    ...
    $.ajax({
      ...
      .done(function (res) {
        if (res.status == "success") {
          ...
          employeeManagerForm[0].reset(); // Reset form
          setTimeout(function () {
            window.location.href = formUrl("admin/manager");
          });
        } else if (res.status == "error") {
          toastr.error(res.message);
        } else {
  ...
  employeeManagerForm.submit(function (e) {
    e.preventDefault();
    if (employeeManagerFormValidator.valid()) {
      saveEmployee();
    }
  });
});

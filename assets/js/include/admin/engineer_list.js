 ...
 */
 ...
 $(function () {
     ...
     const listArea = $('#empengineer--deatils--area');
     const listContainer = listArea.find('[data-container="empEngineerArea"]');
     const listPagination = listArea.find('[data-pagination="empEngineerArea"]');
                     ...
                     if (res.employees) {
                         ...
                         if (details.length && pagination.total > 0) {
                             ...
                             $.each(details, function (listIn, listVal) {
                                 ...
                                 var employeeViewLink = formUrl('admin/engineer/view/' + listVal.employee_id);
                                 var employeeEditLink = formUrl('admin/engineer/edit/' + listVal.employee_id);
                                 listContainer.append(`<div class="col-md-12">
                                     <div class="card m-b-20 card-body" style="padding: 0.75rem;">
                                         <div class="row align-items-center ">
                                             ...
                                             <div class="float-right">
                                                 ...
                                                 <a href="javascript:void(0);" class="btn-delete-employee-engineer" data-employee="${listVal.employee_id}"><i class="fa fa-trash text-danger"></i></a>
                                             </div>
                                             </div>
                                         </div>
                                     </div>
                             ...
                             listPagination.find(".list-pagination").pagination({
                                 ...
                                 onPageClick: function (pageNumber, event) {
                                     var page_link = formApiUrl('admin/employee/engineer/list', { start: ((parseInt(pagination.length) * (pageNumber - 1)) + 1)});
     ...
     searchForm.submit(function (e) {
         e.preventDefault();
         loadEmpDetails(formApiUrl('admin/employee/engineer/list'));  // Load employee details
     });
     ...
     $(listContainer).on('click', '.btn-switch', function (e) {
        ...
        Swal.fire({
            icon: 'question',
            title: 'Are you sure to change employee Status to ' + status_lable,
            allowOutsideClick: false,
            allowEscapeKey: false,
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    url: formApiUrl('admin/employee/engineer/status/update', { employee_id: employee_id }),
                    type: 'post',
                    dataType: 'json',
                    data: {status: status},
                    headers: {
     ...
     // Delete employee
     $(listContainer).on('click', '.btn-delete-employee-engineer', function (e) {
         e.preventDefault();
         var employee_id = $(this).attr('data-employee');
         Swal.fire({
             icon: 'question',
             ...
             if (result.isConfirmed) {
                 ...
                 $.ajax({
                     url: formApiUrl('admin/employee/engineer/delete', { employee_id: employee_id }),
                     type: 'post',
                     dataType: 'json',
                     headers: {
                         Authorization: `Bearer ${wapLogin.getToken()}`
     ...
     $('#btn-download-upload-sample').click(function (e) {
        ...
        $.ajax({
            url: formApiUrl('admin/employee/Engineer/downloadSample'),
            type: 'get',
            dataType: 'json',
            headers: {
                Authorization: `Bearer ${wapLogin.getToken()}`,
            ...
            if (res.status == 'success') {
                if (res.content) {
                    let fileName = 'engineer-' + moment().format('DD/MM/YYYY') + '.xlsx';

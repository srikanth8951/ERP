 ...
 */
 ...
 $(function () {
   ...
   const employeeStoreForm = $("#employeeStoreForm");
   var employeeStoreFormValidator;
   ...
   window.loadAutocompleteRegions = function (selected = []) {
     let moduleRegion = new ModuleRegion({
       ...
       selectboxElement:employeeStoreForm.find('[name="region_id"]'),
     });
   ...
   window.loadAutocompleteBranches = function (selected = []) {
     ...
     let moduleBranch = new ModuleBranch({
       ...
       selectboxElement:employeeStoreForm.find('[name="branch_id"]'),
     });
  ...
  employeeStoreForm.find('[name="region_id"]').on("select2:select", function () {
     if (parseValue($(this).val()) != "") {
       loadAutocompleteBranches({
         region_id: $(this).val(),
       });
     } else {
      employeeStoreForm.find('[name="branch_id"]').html("").trigger("change");
     }
   });
   ...
   window.loadAutocompleteDesignations = function (selected = []) {
     let moduleDesignation = new ModuleDesignation({
       ...
       selectboxElement:employeeStoreForm.find('[name="designation_id"]'),
     });
   ...
   window.loadAutocompleteDepartments = function (selected = []) {
     let moduleDepartment = new ModuleDepartment({
       ...
       selectboxElement:employeeStoreForm.find('[name="department_id"]'),
     });
   ...
   window.loadAutocompleteWorkExpertise = function (selected = []) {
     let moduleWorkExpertise = new ModuleWorkExpertise({
       ...
       selectboxElement:employeeStoreForm.find('[name="work_expertise"]'),
     });
   ...
   // Add user region
  employeeStoreForm.on("click", "#btn-add-user-region", function (e) {
     e.preventDefault();
     let moduleRegion = new ModuleRegion({
       autoloadUrl: formApiUrl("admin/region/autocomplete"),
       selectboxElement:employeeStoreForm.find('[name="region_id"]'),
     ...
     moduleRegion.loadPrompt({
       selected: [employeeStoreForm.find('[name="region_id"]').val()],
       submitAction: formApiUrl("admin/region/add"),
     });
   });
 
   // Add user branch
  employeeStoreForm.on("click", "#btn-add-user-branch", function (e) {
     e.preventDefault();
     let moduleBranch = new ModuleBranch({
       autoloadUrl: formApiUrl("admin/branch/autocomplete"),
       selectboxElement:employeeStoreForm.find('[name="branch_id"]'),
     ...
     moduleBranch.loadPrompt({
       selected: [employeeStoreForm.find('[name="branch_id"]').val()],
  ...
  employeeStoreForm.on("click", "#btn-add-user-designation", function (e) {
     e.preventDefault();
     let moduleDesignation = new ModuleDesignation({
       autoloadUrl: formApiUrl("admin/designation/autocomplete"),
       selectboxElement:employeeStoreForm.find('[name="designation_id"]'),
     ...
     moduleDesignation.loadPrompt({
       selected: [employeeStoreForm.find('[name="designation_id"]').val()],
       submitAction: formApiUrl("admin/designation/add"),
     });
   });
 
   // Add user department
  employeeStoreForm.on("click", "#btn-add-user-department", function (e) {
     e.preventDefault();
     let moduleDepartment = new ModuleDepartment({
       autoloadUrl: formApiUrl("admin/department/autocomplete"),
       selectboxElement:employeeStoreForm.find('[name="department_id"]'),
     ...
     moduleDepartment.loadPrompt({
       selected: [employeeStoreForm.find('[name="department_id"]').val()],
       submitAction: formApiUrl("admin/department/add"),
     });
   });
 
   // Add user WorkExpertise
  employeeStoreForm.on("click", "#btn-add-user-work-expertise", function (e) {
     e.preventDefault();
     let moduleWorkExpertise = new ModuleWorkExpertise({
       autoloadUrl: formApiUrl("admin/work_expertise/autocomplete"),
       selectboxElement:employeeStoreForm.find('[name="work_expertise"]'),
     ...
     moduleWorkExpertise.loadPrompt({
       selected: [employeeStoreForm.find('[name="work_expertise"]').val()],
   ...
   window.loadAutocompleteCountry = function (options = {}) {
     var selected;
     var countrySelectbox =employeeStoreForm.find('[name="country_id"]');
   ...
   window.loadAutocompleteStates = function (options = {}) {
     var selected;
     var country_id;
     var stateSelectbox =employeeStoreForm.find('[name="state_id"]');
   ...
   window.loadAutocompleteCities = function (options = {}) {
     var selected;
     var state_id;
     var citySelectbox =employeeStoreForm.find('[name="city_id"]');
   ...
   if (emp_form_type == "edit") {
    ...
    employeeStoreForm.find('[name="country_id"]').change(function () {
       if (parseValue($(this).val()) != "") {
   ...
   if (emp_form_type == "edit") {
    ...
    employeeStoreForm
     ...
     // passing state ID to city
    employeeStoreForm.find('[name="state_id"]').change(function () {
       if (parseValue($(this).val()) != "") {
         loadAutocompleteCities({
           state_id: $(this).val(),
         });
       } else {
        employeeStoreForm.find('[name="city_id"]').html("").trigger("change");
       }
     });
   }
 
  employeeStoreForm.find(".password-generate").click(function (e) {
     e.preventDefault();
     let res = generateRandomString(8);
    employeeStoreForm.find('[name="password"]').val(res);
   });
       ...
       success: function (res) {
         if (res.status == "success") {
           ...
           if (parseValue(res.employee.data) != "") {
            ...
            employeeStoreForm.find('[name="last_name"]').val(empDetail.last_name);
            employeeStoreForm.find('[name="email"]').val(empDetail.email);
            employeeStoreForm.find('[name="mobile"]').val(empDetail.mobile);
            employeeStoreForm.find('[name="username"]').val(empDetail.username);
            employeeStoreForm.find('[name="address"]').val(empDetail.address);
            employeeStoreForm.find('[name="pincode"]').val(empDetail.pincode);
            employeeStoreForm
   ...
   // employee Form
  employeeStoreFormValidator =employeeStoreForm.validate({
   ...
   if (emp_form_type == "edit") {
    employeeStoreForm.find('[name="password"]').rules("remove");
   } else {
    employeeStoreForm.find('[name="password"]').rules("add", {
       required: true,
       messages: {
     ...
     $.ajax({
       ...
       .done(function (res) {
         if (res.status == "success") {
          ...
          employeeStoreForm[0].reset(); // Reset form
           setTimeout(function () {
             window.location.href = formUrl("admin/store/users");
           });
         } else if (res.status == "error") {
           toastr.error(res.message);
         } else {
  ...
  employeeStoreForm.submit(function (e) {
     e.preventDefault();
     if (employeeStoreFormValidator.valid()) {
       saveEmployee();
     }
   });
 });
